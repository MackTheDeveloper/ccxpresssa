...
class UpsMaster extends Model
{
    ...
    static function checkFileAssgned($upsMasterId)
    {
        $data = DB::table('ups_master')->where('id', $upsMasterId)->first();
        $assigned = '';
    ...
    {
        $data = array();
        $total = DB::table('ups_details')->where('master_ups_id', $id)->where('deleted','0')->get();
        $allDelivered = 0;
        if (count($total) > 0) {
            $allDelivered = 1;
            foreach ($total as $k => $v) {
                if ($v->ups_scan_status != 6 && $v->ups_scan_status != 13 && $v->ups_scan_status != 14) {
                    $allDelivered = 0;
                    break;
                }
            }
    ...
    static function getMasterUpsData($upsMasterId)
    {
        $data = DB::table('ups_master')->where('id', $upsMasterId)->first();
        return $data;
    }
