...
class CcpackExpenseController extends Controller
{
    /**
    ...
    {
        $checkPermission = User::checkPermission(['listing_ccpack_expenses'], '', auth()->user()->id);
        if (!$checkPermission) {
            return redirect('/home');
        }

        return view("ccpack-expenses.index");
    }

    public function listbydatatableserverside(Request $request)
    {
        $permissionExpensesEdit = User::checkPermission(['update_ccpack_expenses'], '', auth()->user()->id);
        $permissionExpensesDelete = User::checkPermission(['delete_ccpack_expenses'], '', auth()->user()->id);
        ...
        $orderby = ['', 'expenses.expense_id', 'expenses.expense_id', 'exp_date', 'voucher_number', 'ccpack.file_number', 'bl_awb', '', 'note', 'expenses.consignee', 'expenses.shipper', 'currency.code', '', '', 'expense_request', 'expense_type'];
        ...
        foreach ($query as $key => $items) {
            ...
            $ccpackCargo = ccpack::getccpackdetail($items->ccpack_id);
            if (!empty($ccpackCargo)) {

                $invoiceOfFile = Expense::getCcpackInvoicesOfFile($items->ccpack_id);
                ...
                $dataClientUsingModuleId = Common::getClientDataUsingModuleId('ccpack', $items->ccpack_id);
                $action = '<div class="dropdown">';

                $delete = route('deleteexpensevoucher', $items->expense_id);
                $edit = route('ccpackexpenseedit', [$items->expense_id]);

                $action .= '<a title="Click here to print"  target="_blank" href="' . route('printoneccpackexpense', [$items->expense_id, $items->ccpack_id]) . '"><i class="fa fa-print"></i></a>&nbsp; &nbsp;';
                if ($items->deleted == '0') {
    ...
    public function create($ccpackId = null, $flagFromWhere = null)
    {
        $checkPermission = User::checkPermission(['add_ccpack_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        $dataAwbNos = DB::table('ccpack')->where('deleted', '0')->get()->pluck('awb_number', 'id');

        $dataFileNumber = DB::table('ccpack')->where('deleted', '0')
        ...
        return view('ccpack-expenses._form', ['model' => $model, 'billingParty' => $billingParty, 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'voucherNo' => $voucherNo, 'dataCost' => $dataCost, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'currency' => $currency, 'cashier' => $cashier, 'ccpackId' => $ccpackId, 'flagFromWhere' => $flagFromWhere, 'adminManagersRole' => $adminManagersRole]);
    }
    ...
    public function store(Request $request)
    {
        //session_start();
        $input = $request->all();
        $fileData = DB::table('ccpack')->where('id', $input['file_number'])->where('deleted', '0')->first();
        $getLastExpense = DB::table('expenses')->orderBy('expense_id', 'desc')->first();
        if (empty($getLastExpense))
        ...
        $input['display_notification_cashier_for_ccpack'] = '1';
        ...
        $dataCcpack = DB::table('ccpack')->where('id', $_POST['ccpack_id'])->first();
        $data['flagExpense'] = 'Ccpack - ' . $dataCcpack->file_number;
        Activities::log('create', 'ccpackexpense', (object) $data);
        ...
        if (isset($input['flagBtn']) && !empty($input['flagBtn']) && $input['flagBtn'] == 'saveprint') {
            ...
            $dataCcpack = DB::table('ccpack')->where('id', $model->ccpack_id)->first();
            $pdf = PDF::loadView('ccpack-expenses.print-one', ['expenseData' => $expenseData, 'dataCcpack' => $dataCcpack]);
            $pdf_file = $dataCcpack->file_number . '_' . $model->expense_id . '_expense.pdf';
            $pdf_path = 'public/ccpackExpensePdf/' . $pdf_file;
            $pdf->save($pdf_path);

            $s3path = 'Files/Courier/CCpack/';
            if ($dataCcpack->ccpack_operation_type == 1) {
                $s3path .= 'Import/' . $dataCcpack->file_number . '/Expenses/';
            } else {
                $s3path .= 'Export/' . $dataCcpack->file_number . '/Expenses/';
            }
    ...
    {
        $checkPermission = User::checkPermission(['update_ccpack_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        return view('ccpack-expenses._formedit', ['model' => $model, 'billingParty' => $billingParty, 'flag' => 'cargo', 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'dataCost' => $dataCost, 'dataExpenseDetails' => $dataExpenseDetails, 'flagFromWhere' => $flagFromWhere, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'dataCcpack' => $dataCcpack, 'currency' => $currency, 'cashier' => $cashier, 'adminManagersRole' => $adminManagersRole, 'expenseStatus' => $expenseStatus]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Activities::log('update', 'ccpackexpense', $model);
        $input = $request->all();
        $input['exp_date'] = !empty($input['exp_date']) ? date('Y-m-d', strtotime($input['exp_date'])) : date('Y-m-d');
    ...
    public function printone($expenseId = null, $ccpackId =  null, $flag = null)
    {
        ...
        $dataCcpack = DB::table('ccpack')->where('id', $ccpackId)->first();
        if (empty($dataCcpack))
            $dataCcpack = new Cargo;
        $pdf = PDF::loadView('ccpack-expenses.print-one', ['expenseData' => $expenseData, 'dataCcpack' => $dataCcpack]);
    ...
    public function printall($flag = null)
    {
        $cargoExpenseData = DB::table('expenses')
            ->select(DB::raw('expenses.*,ccpack.file_number'))
            ->join('ccpack', 'ccpack.id', '=', 'expenses.ccpack_id')
            ->where('expenses.deleted', '0')
            ->whereNotNull('expenses.ccpack_id');
        ...
        if ($flag != 'all') {
            $cargoExpenseData = $cargoExpenseData->where('ccpack_id', $flag)->get()->toArray();
            $cargoData = DB::table('ccpack')->where('id', $flag)->first();
        } else {
        ...
        $pdf = PDF::loadView('ccpack-expenses.print-all', ['cargoExpenseData' => $cargoExpenseData, 'flag' => $flag, 'cargoData' => $cargoData]);
    ...
    public function  getprintviewsingleccpackexpensecashier($expenseId = null, $ccpackId =  null, $flag = null)
    {
        $checkPermission = User::checkPermission(['change_file_expense_status_ccpack'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');

        if ($flag == 'fromNotification')
            Expense::where('expense_id', $expenseId)->update(['display_notification_cashier_for_ccpack' => 0]);
        ...
        return view('ccpack-expenses.getprintviewsingleccpackexpensecashier', ['cargoExpenseData' => $cargoExpenseData, 'expenseId' => $expenseId, 'ccpackId' => $ccpackId, 'expenseStatus' => $expenseStatus, 'cashCredit' => $cashCredit]);
    }

    public function changeccpackexpensestatusbycashier(Request $request)
    {
        session_start();
        $input = $request->all();
        ...
        Session::flash('flash_message', 'Record has been updated successfully');
        return redirect()->route('getprintviewsingleccpackexpensecashier', [$model->expense_id, $model->ccpack_id]);
    }
}
