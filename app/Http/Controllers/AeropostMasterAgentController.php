...
class AeropostMasterAgentController extends Controller
{
    /**
    ...
    {
        $checkPermission = User::checkPermission(['assign_billingparty_cashcredit_aeropost_master'], '', auth()->user()->id);
        if (!$checkPermission) {
        ...
        return view('agent-role.aeropost-master.view', ['model' => $model, 'billingParty' => $billingParty, 'filesInfo' => $attachedFiles, 'fileTypes' => $fileTypes, 'activityData' => $activityData]);
    }

    public function assignOperations(Request $request)
    {
        $input = $request->all();
        $model = AeropostMaster::find($input['id']);
