...
class CcpackMasterAgentController extends Controller
{
    /**
    ...
    {
        $checkPermission = User::checkPermission(['assign_billingparty_cashcredit_ccpack_master'], '', auth()->user()->id);
        if (!$checkPermission) {
        ...
        return view('agent-role.ccpack-master.view', ['model' => $model, 'billingParty' => $billingParty, 'filesInfo' => $attachedFiles, 'fileTypes' => $fileTypes, 'activityData' => $activityData]);
    }

    public function assignOperations(Request $request)
    {
        $input = $request->all();
        $model = CcpackMaster::find($input['id']);
