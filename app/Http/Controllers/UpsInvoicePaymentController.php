...
class UpsInvoicePaymentController extends Controller
{

    public function getshipmentnumbers()
    {
        $dataShipmentNumbers = DB::table('ups_details')->select('shipment_number')->where('deleted', 0)->whereNotNull('shipment_number')->get();
        return json_encode($dataShipmentNumbers);
    }
    public function getupsinvoices()
    {
        $invoiceArray = DB::table('invoices')
            ->select('bill_no', 'id')
            //->whereNotNull('ups_id')
            ->where(function ($query) {
                $query->whereNotNull('ups_id')
    ...
    public function create($upsId = null, $invoceId = null, $billingParty = null, $fromMenu = null)
    {
        $checkPermission = User::checkPermission(['add_courier_invoice_payments'], '', auth()->user()->id);
        if (!$checkPermission)
        ...
        /* $dataShipmentNumbers = DB::table('ups_details')->where('deleted', 0)->whereNotNull('shipment_number')->get()->pluck('shipment_number', 'shipment_number'); */
        $dataShipmentNumbers = array();
        ...
        return view("upsinvoicepayments.form", ['cashCredit' => $cashCredit, 'model' => $model, 'dataFileNumber' => $dataFileNumber, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'upsId' => $upsId, 'invoceId' => $invoceId, 'invoiceArray' => $invoiceArray, 'paymentVia' => $paymentVia, 'billingParty' => $billingParty, 'fromMenu' => $fromMenu, 'currency' => $currency, 'dataShipmentNumbers' => $dataShipmentNumbers]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($input['client']) || !empty($input['invoice_id'])) {
            ...
            foreach ($input['payment'] as $key => $value) {
                ...
                if (!empty($dataInvoice->ups_id)) {
                    $input['flagModule'] = 'UPS';
                } else {
                    $input['flagModule'] = 'UPS Master';
                }

                if ($input['flagModule'] == 'UPS')
                    $dataUps = DB::table('ups_details')->where('id', $dataInvoice->ups_id)->first();
                else
                    $dataUps = DB::table('ups_master')->where('id', $dataInvoice->ups_master_id)->first();
                ...
                if ($input['flagModule'] == 'UPS')
                    ...
                    $input['ups_master_id'] = $dataInvoice->ups_master_id;
                $input['file_number'] = $dataUps->file_number;
                $input['amount'] = $value;
                ...
                if ($value == $dataInvoice->balance_of)
                    DB::table('invoices')->where('id', $key)->update(['invoice_status_changed_by' => auth()->user()->id, 'credits' => $dataInvoice->total, 'payment_status' => 'Paid', 'balance_of' => '0.00', 'payment_received_on' => date('Y-m-d'), 'payment_received_by' => auth()->user()->id]);
                else
        ...
        if (isset($input['flagBtn']) && !empty($input['flagBtn']) && $input['flagBtn'] == 'saveprint') {
            if ($input['flagInvoiceOrClient'] == 'invoice') {
                ...
                if ($input['flagModule'] == 'UPS')
                    $data = $data->whereNotNull('invoice_payments.ups_id');
                else
                    $data = $data->whereNotNull('invoice_payments.ups_master_id');
    ...
    public function getupsinvoicesofclient()
    {
        $clientId = $_POST['clientId'];
        $shipmentNumber = !empty($_POST['shipmentNumber']) ? $_POST['shipmentNumber'] : '';
        $dataInvoice = DB::table('invoices')
            ->select('invoices.*', 'ups_details.shipment_number')
            ->join('ups_details', 'ups_details.id', '=', 'invoices.ups_id')
            ->where('invoices.bill_to', $clientId)->where('invoices.deleted', 0)->where('invoices.payment_status', '!=', 'Paid')->whereNotNull('invoices.ups_id')->where('invoices.total', '!=', '0.00');

        $dataInvoiceUpsMaster = DB::table('invoices')
            ->select('invoices.*')
            ->join('ups_master', 'ups_master.id', '=', 'invoices.ups_master_id')
            ->where('invoices.bill_to', $clientId)->where('invoices.deleted', 0)->where('invoices.payment_status', '!=', 'Paid')->whereNotNull('invoices.ups_master_id')->where('invoices.total', '!=', '0.00')
            ->get()->toArray();

        if (!empty($shipmentNumber)) {
            $dataInvoice = $dataInvoice->where('ups_details.shipment_number', $shipmentNumber);
        }
        $dataInvoice = $dataInvoice->get()->toArray();

        $allInvoices = array_merge($dataInvoice, $dataInvoiceUpsMaster);
        return view("upsinvoicepayments.getclientupsinvoicesajax", ['dataInvoice' => $allInvoices]);
    }

    public function getinvoicesusingfilenumber()
    {
        $upsId = $_POST['upsId'];
        $invoiceNumbers = DB::table('invoices')->where('ups_id', $upsId)->where('payment_status', 'Pending')->where('deleted', '0')->get();
        $dt = '';
    ...
    public function getselectedupsinvoicedata()
    {
        $invoiceId = $_POST['invoiceId'];
        $invoiceData = DB::table('invoices')->where('id', $invoiceId)->first();
        $flagModule = 'UPS';
        if (!empty($invoiceData->ups_id)) {
            $dataInvoice = DB::table('invoices')
                ->select('invoices.*', 'ups_details.shipment_number')
                ->join('ups_details', 'ups_details.id', '=', 'invoices.ups_id')
    ...
    public function getcurrencyratesection()
    {
        ...
        return view("upsinvoicepayments.getcurrencyratesecion", ['currencyExchangeData' => $currencyExchangeData]);
    }
}
