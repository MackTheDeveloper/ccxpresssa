...
class AeropostExpenseController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view("aeropost-expenses.index");
    }

    public function listbydatatableserverside(Request $request)
    {
        $permissionExpensesEdit = User::checkPermission(['update_aeropost_expenses'], '', auth()->user()->id);
        $permissionExpensesDelete = User::checkPermission(['delete_aeropost_expenses'], '', auth()->user()->id);
        ...
        $orderby = ['', 'expenses.expense_id', 'expenses.expense_id', 'exp_date', 'voucher_number', 'aeropost.file_number', 'bl_awb', '', 'note', 'expenses.consignee', 'expenses.shipper', 'currency.code', '', '', 'expense_request', 'expense_type'];
        ...
        foreach ($query as $key => $items) {
            ...
            $aeropostCargo = Aeropost::getAeropostData($items->aeropost_id);
            if (!empty($aeropostCargo)) {

                $invoiceOfFile = Expense::getAeropostInvoicesOfFile($items->aeropost_id);
                ...
                $dataClientUsingModuleId = Common::getClientDataUsingModuleId('aeropost', $items->aeropost_id);
                $action = '<div class="dropdown">';

                $delete = route('deleteexpensevoucher', $items->expense_id);
                $edit = route('aeropostexpenseedit', [$items->expense_id]);

                $action .= '<a title="Click here to print"  target="_blank" href="' . route('printoneaeropostexpense', [$items->expense_id, $items->aeropost_id]) . '"><i class="fa fa-print"></i></a>&nbsp; &nbsp;';
                if ($items->deleted == '0') {
    ...
    public function create($aeropostId = null, $flagFromWhere = null)
    {
        $checkPermission = User::checkPermission(['add_aeropost_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        $dataAwbNos = DB::table('aeropost')->where('deleted', '0')->get()->pluck('tracking_no', 'id');

        $dataFileNumber = DB::table('aeropost')->where('deleted', '0')
        ...
        return view('aeropost-expenses._form', ['model' => $model, 'billingParty' => $billingParty, 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'voucherNo' => $voucherNo, 'dataCost' => $dataCost, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'currency' => $currency, 'cashier' => $cashier, 'aeropostId' => $aeropostId, 'flagFromWhere' => $flagFromWhere, 'adminManagersRole' => $adminManagersRole]);
    }
    ...
    public function store(Request $request)
    {
        //session_start();
        $input = $request->all();
        $fileData = DB::table('aeropost')->where('id', $input['file_number'])->where('deleted', '0')->first();
        $getLastExpense = DB::table('expenses')->orderBy('expense_id', 'desc')->first();
        if (empty($getLastExpense))
        ...
        $input['display_notification_cashier_for_aeropost'] = '1';
        ...
        $dataAeropost = DB::table('aeropost')->where('id', $_POST['aeropost_id'])->first();
        $data['flagExpense'] = 'Aeropost - ' . $dataAeropost->file_number;
        Activities::log('create', 'aeropostexpense', (object) $data);
        ...
        if (isset($input['flagBtn']) && !empty($input['flagBtn']) && $input['flagBtn'] == 'saveprint') {
            ...
            $dataAeropost = DB::table('aeropost')->where('id', $model->aeropost_id)->first();
            $pdf = PDF::loadView('aeropost-expenses.print-one', ['expenseData' => $expenseData, 'dataAeropost' => $dataAeropost]);
            $pdf_file = $dataAeropost->file_number . '_' . $model->expense_id . '_expense.pdf';
            $pdf_path = 'public/aeropostExpensePdf/' . $pdf_file;
            $pdf->save($pdf_path);

            $s3path = 'Files/Courier/Aeropost/' . $fileData->file_number . '/Expenses/';
    ...
    {
        $checkPermission = User::checkPermission(['update_aeropost_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        return view('aeropost-expenses._formedit', ['model' => $model, 'billingParty' => $billingParty, 'flag' => 'cargo', 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'dataCost' => $dataCost, 'dataExpenseDetails' => $dataExpenseDetails, 'flagFromWhere' => $flagFromWhere, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'dataAeropost' => $dataAeropost, 'currency' => $currency, 'cashier' => $cashier, 'adminManagersRole' => $adminManagersRole, 'expenseStatus' => $expenseStatus]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Activities::log('update', 'aeropostexpense', $model);
        $input = $request->all();
        $input['exp_date'] = !empty($input['exp_date']) ? date('Y-m-d', strtotime($input['exp_date'])) : date('Y-m-d');
        ...
        $pdf_path = 'public/aeropostExpensePdf/' . $pdf_file;
        $pdf->save($pdf_path);
        $s3path = 'Files/Courier/Aeropost/' . $dataAeropost->file_number . '/Expenses/';
        $filecontent = file_get_contents($pdf_path);
    ...
    public function printone($expenseId = null, $aeropostId =  null, $flag = null)
    {
        ...
        $dataAeropost = DB::table('aeropost')->where('id', $aeropostId)->first();
        if (empty($dataAeropost))
            $dataAeropost = new Cargo;
        $pdf = PDF::loadView('aeropost-expenses.print-one', ['expenseData' => $expenseData, 'dataAeropost' => $dataAeropost]);
    ...
    public function printall($flag = null)
    {
        $cargoExpenseData = DB::table('expenses')
            ->select(DB::raw('expenses.*,aeropost.file_number'))
            ->join('aeropost', 'aeropost.id', '=', 'expenses.aeropost_id')
            ->where('expenses.deleted', '0')
            ->whereNotNull('expenses.aeropost_id');
        ...
        if ($flag != 'all') {
            $cargoExpenseData = $cargoExpenseData->where('aeropost_id', $flag)->get()->toArray();
            $cargoData = DB::table('aeropost')->where('id', $flag)->first();
        } else {
        ...
        $pdf = PDF::loadView('aeropost-expenses.print-all', ['cargoExpenseData' => $cargoExpenseData, 'flag' => $flag, 'cargoData' => $cargoData]);
    ...
    public function  getprintviewsingleaeropostexpensecashier($expenseId = null, $aeropostId =  null, $flag = null)
    {
        $checkPermission = User::checkPermission(['change_file_expense_status_aeropost'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');

        if ($flag == 'fromNotification')
            Expense::where('expense_id', $expenseId)->update(['display_notification_cashier_for_aeropost' => 0]);
        ...
        return view('aeropost-expenses.getprintviewsingleaeropostexpensecashier', ['cargoExpenseData' => $cargoExpenseData, 'expenseId' => $expenseId, 'aeropostId' => $aeropostId, 'expenseStatus' => $expenseStatus, 'cashCredit' => $cashCredit]);
    }

    public function changeaeropostexpensestatusbycashier(Request $request)
    {
        session_start();
        $input = $request->all();
        ...
        Session::flash('flash_message', 'Record has been updated successfully');
        return redirect()->route('getprintviewsingleaeropostexpensecashier', [$model->expense_id, $model->aeropost_id]);
    }
}
