...
class CcpackMasterExpensesController extends Controller
{
    /**
    ...
    {
        $checkPermission = User::checkPermission(['listing_ccpack_master_expenses'], '', auth()->user()->id);
        if (!$checkPermission) {
            return redirect('/home');
        }

        return view("ccpack-master-expenses.index");
    }

    public function listbydatatableserverside(Request $request)
    {
        $permissionExpensesEdit = User::checkPermission(['update_ccpack_master_expenses'], '', auth()->user()->id);
        $permissionExpensesDelete = User::checkPermission(['delete_ccpack_master_expenses'], '', auth()->user()->id);
        ...
        $orderby = ['', 'expenses.expense_id', 'expenses.expense_id', 'exp_date', 'voucher_number', 'ccpack_master.file_number', 'bl_awb', '', 'note', 'expenses.consignee', 'expenses.shipper', 'currency.code', '', '', 'expense_request', 'expense_type'];
        ...
        foreach ($query as $key => $row) {
            ...
            $query1[$key] = $row->ccpack_master_id;
        }
        ...
        foreach ($query as $key => $items) {
            ...
            $ccpackCargo = CcpackMaster::getMasterCcpackData($items->ccpack_master_id);
            if (!empty($ccpackCargo)) {

                $invoiceOfFile = Expense::getCcpackMasterInvoicesOfFile($items->ccpack_master_id);
                ...
                $dataClientUsingModuleId = Common::getClientDataUsingModuleId('ccpackMaster', $items->ccpack_master_id);
                $action = '<div class="dropdown">';

                $delete = route('deleteexpensevoucher', $items->expense_id);
                $edit = route('editccpackmasterexpense', [$items->expense_id]);

                $action .= '<a title="Click here to print"  target="_blank" href="' . route('printsingleccpackmasterexpense', [$items->expense_id, $items->ccpack_master_id]) . '"><i class="fa fa-print"></i></a>&nbsp; &nbsp;';
                if ($items->deleted == '0') {
                    if ($permissionExpensesEdit && $ccpackCargo->file_close != 1) {
                ...
                $data1[] = [$checkBoxes, $items->expense_id, '', date('d-m-Y', strtotime($items->exp_date)), $items->voucher_number, $items->expense_type == 1 ? 'Cash' : 'Credit', $ccpackCargo->file_number, $items->bl_awb, !empty($currencyData) ? '(' . $currencyData->currency_code . ')' . ' ' . $currencyData->name : '-', $items->note != '' ? $items->note : '-', $dataClientUsingModuleId['consigneeName'], $dataClientUsingModuleId['shipperName'], !empty($dataCurrency->code) ? $dataCurrency->code : "-", $totlaExpense, $invoiceOfFile, $items->expense_request, $action];
            }
        }
        $json_data = array(
    ...
    {
        $checkPermission = User::checkPermission(['add_ccpack_master_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        $dataAwbNos = DB::table('ccpack_master')->where('deleted', '0')->get()->pluck('tracking_number', 'id');

        $dataFileNumber = DB::table('ccpack_master')->where('deleted', '0')
        ...
        return view('ccpack-master-expenses._form', ['model' => $model, 'billingParty' => $billingParty, 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'voucherNo' => $voucherNo, 'dataCost' => $dataCost, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'currency' => $currency, 'cashier' => $cashier, 'ccpackMasterId' => $ccpackMasterId, 'flagFromWhere' => $flagFromWhere, 'adminManagersRole' => $adminManagersRole]);
    }
    ...
    public function store(Request $request)
    {
        //session_start();
        $input = $request->all();
        $fileData = DB::table('ccpack_master')->where('id', $input['file_number'])->where('deleted', '0')->first();
        $getLastExpense = DB::table('expenses')->orderBy('expense_id', 'desc')->first();
        if (empty($getLastExpense))
        ...
        $input['display_notification_cashier_for_ccpack_master'] = '1';
        ...
        $dataCcpack = DB::table('ccpack_master')->where('id', $_POST['ccpack_master_id'])->first();
        $data['flagExpense'] = $dataCcpack->file_number;
        Activities::log('create', 'ccpackMasterExpense', (object) $data);
        ...
        if (isset($input['flagBtn']) && !empty($input['flagBtn']) && $input['flagBtn'] == 'saveprint') {
            ...
            $dataCcpack = DB::table('ccpack_master')->where('id', $model->ccpack_master_id)->first();
            $pdf = PDF::loadView('ccpack-master-expenses.print-one', ['expenseData' => $expenseData, 'dataCcpack' => $dataCcpack]);
            $pdf_file = $dataCcpack->file_number . '_' . $model->expense_id . '_expense.pdf';
            $pdf_path = 'public/ccpackMasterExpensePdf/' . $pdf_file;
            $pdf->save($pdf_path);

            $s3path = 'Files/Courier/Ccpack-Master/' . $fileData->file_number . '/Expenses/';
    ...
    {
        $checkPermission = User::checkPermission(['update_ccpack_master_expenses'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');
        ...
        return view('ccpack-master-expenses._formedit', ['model' => $model, 'billingParty' => $billingParty, 'flag' => 'cargo', 'dataAwbNos' => $dataAwbNos, 'dataFileNumber' => $dataFileNumber, 'dataCost' => $dataCost, 'dataExpenseDetails' => $dataExpenseDetails, 'flagFromWhere' => $flagFromWhere, 'allUsers' => $allUsers, 'cashCredit' => $cashCredit, 'dataCcpack' => $dataCcpack, 'currency' => $currency, 'cashier' => $cashier, 'adminManagersRole' => $adminManagersRole, 'expenseStatus' => $expenseStatus]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Activities::log('update', 'ccpackMasterExpense', $model);
        $input = $request->all();
        $input['exp_date'] = !empty($input['exp_date']) ? date('Y-m-d', strtotime($input['exp_date'])) : date('Y-m-d');
        ...
        $pdf_path = 'public/ccpackMasterExpensePdf/' . $pdf_file;
        $pdf->save($pdf_path);
        $s3path = 'Files/Courier/Ccpack-Master/' . $dataCcpack->file_number . '/Expenses/';
        $filecontent = file_get_contents($pdf_path);
        $success = Storage::disk('s3')->put($s3path . $dataCcpack->file_number . '_' . $model->voucher_number . '_expense.pdf', $filecontent, 'public');
    ...
    public function print($expenseId = null, $ccpackMasterId =  null, $flag = null)
    {
        ...
        $dataCcpack = DB::table('ccpack_master')->where('id', $ccpackMasterId)->first();
        if (empty($dataCcpack))
            $dataCcpack = new CcpackMaster;
        $pdf = PDF::loadView('ccpack-master-expenses.print-one', ['expenseData' => $expenseData, 'dataCcpack' => $dataCcpack]);
        $pdf_file = $dataCcpack->file_number . '_' . $expenseId . '_expense.pdf';
        $pdf_path = 'public/ccpackMasterExpensePdf/' . $pdf_file;
        $pdf->save($pdf_path);
        $s3path = 'Files/Courier/Ccpack-Master/' . $dataCcpack->file_number . '/Expenses/';
        $filecontent = file_get_contents($pdf_path);
        $success = Storage::disk('s3')->put($s3path . $dataCcpack->file_number . '_' . $expenseData[0]->voucher_number . '_expense.pdf', $filecontent, 'public');
    ...
    public function printall($flag = null)
    {
        $cargoExpenseData = DB::table('expenses')
            ->select(DB::raw('expenses.*,ccpack_master.file_number'))
            ->join('ccpack_master', 'ccpack_master.id', '=', 'expenses.ccpack_master_id')
            ->where('expenses.deleted', '0')
            ->whereNotNull('expenses.ccpack_master_id');
        ...
        if ($flag != 'all') {
            $cargoExpenseData = $cargoExpenseData->where('ccpack_master_id', $flag)->get()->toArray();
            $cargoData = DB::table('ccpack_master')->where('id', $flag)->first();
        } else {
        ...
        $pdf = PDF::loadView('ccpack-master-expenses.print-all', ['cargoExpenseData' => $cargoExpenseData, 'flag' => $flag, 'cargoData' => $cargoData]);
    ...
    public function  viewccpackmasterexpenseforcashier($expenseId = null, $ccpackMasterId =  null, $flag = null)
    {
        $checkPermission = User::checkPermission(['change_file_expense_status_ccpack_master'], '', auth()->user()->id);
        if (!$checkPermission)
            return redirect('/home');

        if ($flag == 'fromNotification')
            Expense::where('expense_id', $expenseId)->update(['display_notification_cashier_for_ccpack_master' => 0]);
        ...
        return view('ccpack-master-expenses.getprintviewsingleccpackexpensecashier', ['cargoExpenseData' => $cargoExpenseData, 'expenseId' => $expenseId, 'ccpackMasterId' => $ccpackMasterId, 'expenseStatus' => $expenseStatus, 'cashCredit' => $cashCredit]);
    }

    public function changeccpackmasterexpensestatusbycashier(Request $request)
    {
        session_start();
        $input = $request->all();
        ...
        Session::flash('flash_message', 'Record has been updated successfully');
        return redirect()->route('viewccpackmasterexpenseforcashier', [$model->expense_id, $model->ccpack_master_id]);
    }
}
