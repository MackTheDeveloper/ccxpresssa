...
<script type="text/javascript">
    ...
    function DatatableInitiate(fromDate = '', toDate = '') {
        $('#example').DataTable({
            ...
            "scrollX": true,
            "ajax": {
                url: "{{url('ups-master/invoice/listbydatatableserverside')}}", // json datasource
                "data": {
                    "fromDate": fromDate,
                    "toDate": toDate
                },
            ...
            "createdRow": function(row, data, index) {
                ...
                var url = '<?php echo url("ups-master/invoice/checkoperationfordatatableserverside"); ?>';
                $.ajax({
                    url: url,
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        'invoiceId': invoiceId,
                        'flag': 'getMasterUpsInvoiceData'
                    },
                    success: function(dataR) {
                        if (dataR.deleted == '0') {
                            var editLink = '<?php echo url("ups-master/invoice/viewupsmasterinvoicedetails"); ?>';
                            editLink += '/' + invoiceId;
