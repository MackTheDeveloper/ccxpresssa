			...
			<h4 style="background: #efeaeacc;padding: 10px;">House Files Listing</h4>
				<table id="housefile" class="display nowrap" style="width:100%">
					...
					<tbody>
						...
						<tr data-editlink="{{ route('courieraeropostwarehouseflow',[$model->id,$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
							<td>{{$items->file_number}}</td>
							<td>{{$items->billingParty}}</td>
							<td>{{isset(Config::get('app.ups_new_scan_status')[!empty($items->aeropost_scan_status) ? $items->aeropost_scan_status : '-']) ? Config::get('app.ups_new_scan_status')[!empty($items->aeropost_scan_status) ? $items->aeropost_scan_status : '-'] : '-'}}</td>
