...
<section class="content-header">
    <h1>UPS Files Listing<button style="float: right;width: 13%;" id="btnAssignDeliveryBoy" class="btn btn-success btnAssignDeliveryBoy" value="{{route('assign-delivery-boy',['ups'])}}">Assign Delivery Boy</button></h1>
</section>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        if(DataTableState){
            ...
            DatatableInitiate(DataTableState.fromDate, DataTableState.toDate, DataTableState.warehouse, DataTableState.billing_term, DataTableState.upsscan);
        }else{
            DatatableInitiate();
        }
        ...
        $('#createInvoiceForm').validate({
            submitHandler: function(form) {
                ...
                DatatableInitiate(fromDate, toDate, warehouseStatus, billingTerm, fileStatus);
            },
        });
    ...
    function DatatableInitiate(fromDate = '', toDate = '', warehouse = '', billingTerm = '', fileStatus = '') {
        var i = 1;
        var table = $('#example').DataTable({
            ...
            "ajax": {
                url: "{{url('warehouse/ups/listbydatatableserverside')}}", // json datasource
                data: function(d) {
                                    ...
                                    success: function(data) {
                                        if (data.deleted == '0') {
                                            ...
                                            if (masterFileId == '' || masterFileId == null) {
                                                var editLink = '<?php echo url("warehouseups/viewcourierdetailforwarehouse"); ?>';
                                                ...
                                                var editLink = '<?php echo url("warehouseups/courierupswarehouseflow"); ?>';
                                                //editLink += '/' + masterFileId;
                                                editLink += '/' + masterFileId + '/' + UpsId;
                                                $(row).attr('data-editlink', editLink);
                                                $(row).addClass('edit-row');
                                                //$(row).attr('id', masterFileId);
                                                $(row).attr('id', UpsId);
                                            ...
                                            $.ajax({
                                                ...
                                                success: function(data) {
                                                    if (data > 0) {
                                                        ...
                                                        $('td', row).eq(2).attr('style', 'display: block;text-align: center;padding-top: 15px;');
                                                        $('td', row).eq(2).attr('data-upsid', UpsId);
