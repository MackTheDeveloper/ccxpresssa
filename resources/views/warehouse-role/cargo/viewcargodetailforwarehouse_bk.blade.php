...
<section class="content editupscontainer">
	<div class="box box-success">
		<div class="box-body">
			...
			<?php if ($model->file_close != 1) { ?>
				<input type="hidden" class="cargoID" name="id" value="<?php echo $model->id; ?>">
				<div class="col-md-12 detail-container" style="margin-bottom: 10px">
					<div class="col-md-3 row">
						<div class="form-group {{ $errors->has('cargo_master_scan_status') ? 'has-error' :'' }}">
							...
							<div class="col-md-12">
								<?php echo Form::select('cargo_master_scan_status', Config::get('app.ups_new_scan_status'), $model->cargo_master_scan_status, ['class' => 'form-control selectpicker fagent_id', 'data-live-search' => 'true', 'id' => 'cargo_master_scan_status', 'data-container' => 'body']); ?>
							</div>
						</div>
					</div>
					...
					<div class="form-group col-md-2">
						...
						<div class="col-md-12">
							<button type="button" class="btn btn-success assignCargoMasterFileStatus" style="width: 50%">Save</button>
						</div>
					</div>
				</div>
			<?php } ?>
			...
			<?php if (count($HouseAWBData) != 0) { ?>
				...
				foreach ($HouseAWBData as $k => $items) { ?>
					...
					<div class="detail-container detail-container-hawb detail-container-hawb-<?php echo $items->id; ?> detail-container-hawb-<?php echo $i; ?>" style="display:none">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.assignCargoMasterFileStatus').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			...
			var cargo_master_scan_status = $('#cargo_master_scan_status option:selected').val();
			var reason_for_return = $('.reason_for_return option:selected').val();
			var shipment_notes_for_return = $('.shipment_notes_for_return').val();
			var urlz = '<?php echo url("cargo/assigncargomasterfilestatusbywarehouse"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
					'id': id,
					'cargo_master_scan_status': cargo_master_scan_status,
					'reason_for_return': reason_for_return,
					'shipment_notes_for_return': shipment_notes_for_return,
				},
				success: function(data) {
					$('#loading').hide();
					$('.selectpicker').selectpicker('refresh');
