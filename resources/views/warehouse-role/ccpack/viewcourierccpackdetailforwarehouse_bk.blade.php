...
<section class="content-header">
	<h1>
		<div style="float: left"><?php echo 'CCPack - ' . $items->file_number; ?></div>
		<?php if ($items->file_close == 1) { ?>
			<div style="color:red;float:right">CLOSED</div>
		<?php } else { ?>
			<div style="float: right;margin-right: 15px;color: green;"> File Status : <?php echo Config::get('app.ups_new_scan_status')[!empty($items->ccpack_scan_status) ? $items->ccpack_scan_status : '1']; ?>
			</div>
		<?php } ?>
	</h1>
</section>
	...
	<div class="box box-success">
		<div class="box-body">
			...
			<?php if (checkNonBoundedWH() == 'Yes' && $items->display_notification_nonbounded_wh == 1) { ?>
				<a style="margin-top: 10px" class="btn btn-success" href="{{route('acceptfilessubmit',[$items->id,'CCPack','ccpack'])}}">Receive Shipment</a>
			<?php } else {
			?>
				...
				<div class="col-md-12 row">
					<div class="col-md-3 row">
						<div class="form-group {{ $errors->has('ccpack_scan_status') ? 'has-error' :'' }}">
							...
							<div class="col-md-12">
								<?php echo Form::select('ccpack_scan_status', Config::get('app.ups_new_scan_status'), $items->ccpack_scan_status, ['class' => 'form-control selectpicker fagent_id', 'data-live-search' => 'true', 'id' => 'ccpack_scan_status', 'data-container' => 'body']); ?>
							</div>
						</div>
					</div>
		...
		$('.generatehousefileinvoice').click(function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					var locationHref = '<?php echo url("warehousccpack/viewcourierccpackdetailforwarehouse"); ?>';
