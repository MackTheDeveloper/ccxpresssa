	...
	<div class="box box-success">
		<div class="box-body">
			...
			if (count($HouseAWBData) != 0) { ?>
				...
				foreach ($HouseAWBData as $k => $items) { ?>
					<div id="div_basicdetails-<?php echo $i; ?>" class="notes box-s" style="margin-top:12px;margin-bottom:0px;background-color: #365b7b;border-color: #365b7b;">
						<div style="float:left">House File Details</div>
					...
					<div class="detail-container detail-container-hawb detail-container-hawb-<?php echo $i; ?>" style="display:none">
						<div class="row-<?php echo $i; ?>">
							<div class="col-md-12" style="padding:0px">
								...
								<div class="col-md-12 basicDetaiCls" style="padding: 0px;margin-bottom: 15px;">
									...
									<div style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">AWB Number : </span>
										<span class="viewblk2"><?php echo $items->awb_number; ?></span>
									</div>
									<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">Expediteur / Shipper : </span>
										<span class="viewblk2">
											<?php $data = app('App\Clients')->getClientData($items->shipper_name);
									...
									<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
										...
										<span class="viewblk2"><?php echo $items->shipper_telephone; ?></span>
									</div>
									<div style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">Consignee Phone No : </span>
										<span class="viewblk2"><?php echo $items->consignee_telephone; ?></span>
									</div>
									<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">Shipper Address : </span>
										<span class="viewblk2"><?php echo !empty($items->shipper_address) ? $items->shipper_address : '-'; ?></span>
									</div>
									<div style="float: left;width: 50%; margin-bottom: 10px;">
									...
									<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">Weight : </span>
										<span class="viewblk2"><?php echo !empty($items->weight) ? $items->weight : '-'; ?></span>
									</div>
									<div style="float: left;width: 50%; margin-bottom: 10px;">
										<span class="viewblk1">Number of Pieces : </span>
										<span class="viewblk2"><?php echo !empty($items->no_of_pcs) ? (int) $items->no_of_pcs : '-' ?></span>
									</div>
									<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
								...
								<div class="col-md-12" style="padding: 0px;margin-bottom: 4px;background: #f3f3f3;">
									<?php
									$actionUrl = url('warehouseccpack/assignstatusbywarehousecourierccpack');
									?>
									...
									<div class="col-md-8">
										<div class="col-md-3 row">
											<div class="form-group {{ $errors->has('ccpack_scan_status') ? 'has-error' :'' }}">
												<div class="col-md-12">
													<?php echo Form::label('ccpack_scan_status', 'File Status', ['class' => 'control-label']); ?>
												</div>
												<div class="col-md-12">
													<?php echo Form::select('ccpack_scan_status', Config::get('app.ups_new_scan_status'), $items->ccpack_scan_status, ['class' => 'form-control selectpicker ccpack_scan_status-main-' . $items->id, 'data-live-search' => 'true', 'data-container' => 'body']); ?>
												</div>
											</div>
										</div>
										<div class="form-group col-md-3">
											...
											<div class="col-md-12">
												<button type="submit" class="btn btn-success btn-assignstatusbywarehousecouriermaster btn-assignstatusbywarehousecouriermaster-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" style="width: 50%">Save</button>
											</div>
										</div>

									</div>
									<div class="col-md-4" style="display:none;text-align:right;margin-top: 10px;color: green;"> File Status : <?php echo isset(Config::get('app.ups_new_scan_status')[!empty($items->ccpack_scan_status) ? $items->ccpack_scan_status : '1']) ? Config::get('app.ups_new_scan_status')[!empty($items->ccpack_scan_status) ? $items->ccpack_scan_status : '1'] : '-'; ?>
									</div>
								...
								<div class="col-md-12" style="padding: 0px 0px;margin-bottom: 15px;">
									...
									<div class="detail-container">
										<?php $actionUrl = url('warehouse/ccpack/ccpackstep1shipmentstatus'); ?>
										{{ Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form client-side','id'=>'ccpackstep1shipmentstatus','autocomplete'=>'off')) }}
										{{ csrf_field() }}
										<div style="margin-top:0px;float:left;width:100%">
											...
											<div style="width:10%;float:left;margin-right:10px;<?php echo $items->shipment_status == 1 ? 'display:block' : 'display:none'; ?>" class="shipment_received_date_div-<?php echo $items->id; ?>">
												<?php echo Form::text('shipment_received_date', !empty($items->shipment_received_date) ? date('d-m-Y', strtotime($items->shipment_received_date)) :  date('d-m-Y'), ['class' => 'form-control datepicker shipment_received_date shipment_received_date-' . $items->id, 'data-id' => $items->id, 'placeholder' => 'Enter Date']); ?>
											</div>
											...
											<div style="width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep1shipmentstatus btn-ccpackstep1shipmentstatus-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" style="border-radius: 0px;height: 34px;">Save</button>
											</div>
											...
											$dataComments = DB::table('verification_inspection_notes')->where('flag_note', 'V')->where('ccpack_id', $items->id)->orderBy('id', 'desc')->get();
											...
											<div class="ccpackstep1shipmentstatus-ajax-container ccpackstep1shipmentstatus-ajax-container-<?php echo $items->id; ?>" style="float:left;width:100%;margin-top:15px">
									...
									<div class="detail-container">
										<?php $actionUrl = url('warehouse/ccpack/ccpackstep2custominspection'); ?>
										{{ Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form client-side','id'=>'ccpackstep2custominspection','autocomplete'=>'off')) }}
										{{ csrf_field() }}
										<div style="margin-top:0px;float:left;width:100%">
											...
											<div style="width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep2custominspection btn-ccpackstep2custominspection-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" style="border-radius: 0px;height: 34px;">Save</button>
											</div>
											...
											$dataComments = DB::table('verification_inspection_notes')->where('flag_note', 'I')->where('ccpack_id', $items->id)->orderBy('id', 'desc')->get();
											if (count($dataComments) > 0)
												$ajaxData['comments'] = $dataComments;
											else
												$ajaxData['comments'] = '';
											...
											<div class="ccpackstep2custominspection-ajax-container ccpackstep2custominspection-ajax-container-<?php echo $items->id; ?>" style="float:left;width:100%;margin-top:15px">
												<div style="float:left;width:100%;margin-bottom:10px;">
									...
									<div class="detail-container">
										...
										<?php $actionUrl = url('warehouse/ccpack/ccpackstep3movetononboundedwh'); ?>
										{{ Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form client-side','id'=>'ccpackstep3movetononboundedwh','autocomplete'=>'off')) }}
										{{ csrf_field() }}
										<div style="margin-top:0px;float:left;width:100%">
											...
											<div style="width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep3movetononboundedwh btn-ccpackstep3movetononboundedwh-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" style="border-radius: 0px;height: 34px;">Save</button>
											</div>
											...
											<div class="ccpackstep3movetononboundedwh-ajax-container ccpackstep3movetononboundedwh-ajax-container-<?php echo $items->id; ?>" style="float:left;width:100%;margin-top:15px">
												<div style="float:left;width:100%;margin-bottom:10px;">
									...
									<?php
									//$checkInvoiceIsGeneratedOrNot = app('App\Invoices')->checkInvoiceIsGeneratedOrNot($items->id, 'ccpack');
									$checkInvoiceIsGeneratedOrNot = app('App\Invoices')->checkAllInvoiceIsGeneratedOrNot($items->id, 'ccpack');
									$checkIfAnyInvoicePending = app('App\Invoices')->checkIfAnyInvoicePending($items->id, 'ccpack');
									?>
									...
									<div class="detail-container">
										...
										<?php } ?>
										<?php $actionUrl = url('warehouse/ccpack/ccpackstep4invoiceandpayment'); ?>
										{{ Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form client-side','id'=>'ccpackstep4invoiceandpayment','autocomplete'=>'off')) }}
										{{ csrf_field() }}
										<div style="margin-top:0px;float:left;width:100%">

											<div style="width:50%;margin-bottom:10px;margin-top:10px;float:left">
												<?php
												$getTotalDaysOfCharge = app('App\Invoices')->getTotalDaysOfCharge($items->id, 'ccpack');
												$getTotalChargableDays = app('App\Invoices')->getTotalChargableDays($items->id, 'ccpack');
												$getTotalBilledDays = app('App\Invoices')->getTotalBilledDays($items->id, 'ccpack');
												?>
											...
											<div style="width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep4invoiceandpayment btn-ccpackstep4invoiceandpayment-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" style="border-radius: 0px;height: 34px;">Save</button>
											</div>
														...
														<tbody>
															<?php foreach ($checkInvoiceIsGeneratedOrNot as $k => $v) { ?>
																<tr>
																	...
																	<td><a style=" margin-left: 10px;" title="Click here to print" target="_blank" href="{{ route('printinvoice',[$v->id,'ccpack']) }}"><span style="border-radius:0px" class="btn btn-success"><i class="fa fa-print"></i> Print</span></a></td>
																</tr>
															<?php } ?>
														</tbody>
													</table>
									...
									if (!empty($checkInvoiceIsGeneratedOrNot) && $checkInvoiceIsGeneratedOrNot->payment_status == 'Paid') {
										...
										$ajaxData['changedBy'] = '-';
											...
											<!-- <div class="ccpackstep4invoiceandpayment-ajax-container ccpackstep4invoiceandpayment-ajax-container-<?php //echo $items->id; 
									...
									<div class="detail-container">
										<?php $actionUrl = url('warehouse/ccpack/ccpackstep5assigndeliveryboy'); ?>
										{{ Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form client-side','id'=>'ccpackstep3movetononboundedwh','autocomplete'=>'off')) }}
										{{ csrf_field() }}
										<div style="margin-top:0px;float:left;width:100%">
											<div style="width:15%;float:left;margin-right:10px;">
											...
											<div style="width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep5assigndeliveryboy btn-ccpackstep5assigndeliveryboy-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" data-reason='0' style="border-radius: 0px;height: 34px;">Save</button>
											</div>
											...
											<div class="ccpackstep5assigndeliveryboy-ajax-container ccpackstep5assigndeliveryboy-ajax-container-<?php echo $items->id; ?>" style="float:left;width:100%;margin-top:15px">
												<div style="float:left;width:100%;margin-bottom:10px;">
											...
											<div style="width:15%;float:left;margin-right:10px;">
												<?php echo Form::select('ccpack_scan_status', Config::get('app.ups_new_scan_status'), $items->ccpack_scan_status, ['class' => 'form-control selectpicker ccpack_scan_status ccpack_scan_status-' . $items->id, 'data-id' => $items->id, 'data-live-search' => 'true', 'data-container' => 'body', 'placeholder' => 'Select File Status']); ?>
											</div>
											<div style="width:30%;float:left;margin-right:10px;<?php echo $items->ccpack_scan_status == 7 ? 'display:block' : 'display:none' ?>" class="reason_for_return_div-<?php echo $items->id; ?>">
											...
											<div style=" width:5%;float:left;">
												<button type="button" class="btn btn-success btn-ccpackstep5assigndeliveryboy btn-ccpackstep5assigndeliveryboy-<?php echo $items->id; ?>" data-id="<?php echo $items->id; ?>" data-reason='1' style="border-radius: 0px;height: 34px;">Save</button>
											</div>

											<?php
											$ajaxData = array();
											$dataComments = DB::table('verification_inspection_notes')->where('flag_note', 'R')->where('ccpack_id', $items->id)->orderBy('id', 'desc')->get();
											if (count($dataComments) > 0)
												$ajaxData['comments'] = $dataComments;
											else
												$ajaxData['comments'] = '';
											...
											<div class="ccpackstep5assigndeliveryboy-reason-ajax-container ccpackstep5assigndeliveryboy-reason-ajax-container-<?php echo $items->id; ?>" style="float:left;width:100%;margin-top:15px">
												<div style="float:left;width:100%">
													<h3 style="background: #efefef;border-bottom: 1px solid #000;padding: 10px;width: 100%;float: left;font-size: 15px;margin:0px">Comments</h3>
													<?php if (!empty($ajaxData['comments'])) {
														foreach ($ajaxData['comments'] as $k => $v) { ?>
			...
			<?php /* if (checkNonBoundedWH() == 'Yes' && $items->display_notification_nonbounded_wh == 1) { ?>
				<a style="margin-top: 10px" class="btn btn-success" href="{{route('acceptfilessubmit',[$items->id,'ccpack','ccpack_details'])}}">Receive Shipment</a>
			<?php } else { }*/
			?>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.btn-ccpackstep1shipmentstatus').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
			...
			$('#loading').show();
			var urlz = '<?php echo url("warehouse/ccpack/ccpackstep1shipmentstatus"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
				...
				success: function(data) {
					$('#loading').hide();
					$('.ccpackstep1shipmentstatus-ajax-container-' + tId).html(data);
		...
		$('.btn-ccpackstep2custominspection').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			...
			$('#loading').show();
			var urlz = '<?php echo url("warehouse/ccpack/ccpackstep2custominspection"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
				...
				success: function(data) {
					$('#loading').hide();
					$('.ccpackstep2custominspection-ajax-container-' + tId).html(data);
					$('.selectpicker').selectpicker('refresh');
					Lobibox.notify('info', {
						size: 'mini',
						delay: 2000,
		...
		$('.btn-ccpackstep3movetononboundedwh').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			...
			$('#loading').show();
			var urlz = '<?php echo url("warehouse/ccpack/ccpackstep3movetononboundedwh"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
				...
				success: function(data) {
					$('#loading').hide();
					$('.ccpackstep3movetononboundedwh-ajax-container-' + tId).html(data);
					$('.selectpicker').selectpicker('refresh');
					Lobibox.notify('info', {
						size: 'mini',
						delay: 2000,
		...
		$('.btn-ccpackstep4invoiceandpayment').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			...
			$('#loading').show();
			var urlz = '<?php echo url("warehouse/ccpack/ccpackstep4invoiceandpayment"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
		...
		$('.btn-ccpackstep5assigndeliveryboy').click(function(e) {
			e.preventDefault();
			$.ajaxSetup({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			...
			if (reason == 0) {
				...
				var urlz = '<?php echo url("warehouse/ccpack/ccpackstep5assigndeliveryboy"); ?>';
				$.ajax({
					url: urlz,
					type: 'POST',
					data: {
					...
					success: function(data) {
						$('#loading').hide();
						$('.ccpackstep5assigndeliveryboy-ajax-container-' + tId).html(data);
						$('.selectpicker').selectpicker('refresh');
						Lobibox.notify('info', {
							size: 'mini',
							delay: 2000,
				...
				var ccpack_scan_status = $('.ccpack_scan_status-' + tId + " option:selected").val();
				...
				$.ajax({
					...
					data: {
						'id': id,
						'ccpack_scan_status': ccpack_scan_status,
					...
					success: function(data) {
						$('#loading').hide();
						$('.ccpackstep5assigndeliveryboy-reason-ajax-container-' + tId).html(data);
						$('.selectpicker').selectpicker('refresh');
						Lobibox.notify('info', {
							size: 'mini',
							delay: 2000,
		...
		$('.btn-assignstatusbywarehousecouriermaster').click(function(e) {
			...
			$('#loading').show();
			var id = tId;
			var ccpack_scan_status = $('.ccpack_scan_status-main-' + tId + " option:selected").val();
			var urlz = '<?php echo url("warehouseccpack/assignstatusbywarehousecourierccpackmaster"); ?>';
			$.ajax({
				url: urlz,
				type: 'POST',
				data: {
		...
		$('.generatehousefileinvoice').click(function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					var locationHref = '<?php echo url("warehouseccpack/viewcourierccpackdetailforwarehouse"); ?>';
