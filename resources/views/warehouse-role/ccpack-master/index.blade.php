...
<section class="content-header">
    <h1>CCPack Master Files Listing<button style="float: right;width: 13%;" id="btnAssignDeliveryBoy" class="btn btn-success btnAssignDeliveryBoy" value="{{route('assign-delivery-boy',['ccpack'])}}">Assign Delivery Boy</button></h1>
</section>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).delegate('.expandpackage', 'click', function() {
            ...
            if (thiz.hasClass('fa-plus')) {
                ...
                var masterCcpackId = $(this).data('masterccpackid');
                var rowId = $(this).data('rowid');
                $.ajax({
                    url: 'warehouse/ccpack-master/expandhousefiles',
                    type: 'POST',
                    data: {
                        masterCcpackId: masterCcpackId,
                        rowId: rowId
                    },
    ...
    function DatatableInitiate(fromDate = '', toDate = '', fileType = '') {
        ...
        var table = $('#example').DataTable({
            ...
            "ajax": {
                url: "{{url('ccpack-master/listingmasterccpack')}}", // json datasource
                data: function(d) {
                    d.fileType = fileType;
                    d.fromDate = fromDate;
                    d.toDate = toDate;
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data.deleted == '0') {
                                    ...
                                    var editLink = '<?php echo url("warehouseccpack/viewcourierccpackdetailforwarehousemaster"); ?>';
                                    editLink += '/' + MasterCcpackId;
                                    $(row).attr('data-editlink', editLink);
                                    $(row).addClass('edit-row');
                                    $(row).attr('id', MasterCcpackId);
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(data) {
                                            ...
                                            if (data.total > 0) {
                                                ...
                                                $('td', row).eq(1).attr('data-masterccpackid', MasterCcpackId);
