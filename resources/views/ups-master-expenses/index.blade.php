...
@section('title')
UPS Master File Expense Listing
@stop
<?php
...
<section class="content-header">
    <h1>UPS Master File Expense Listing</h1>
</section>
            ...
            <div class="row" style="margin-bottom:20px">
                ...
                <?php if (checkloggedinuserdata() != 'Agent' && checkloggedinuserdata() != 'Cashier') { ?>
                    <div style="float: right;margin-right: 10px;height: 35px;z-index: 111;top: 18px;">
                        <a style="float:left;margin-right: 10px;" title="Click here to print" target="_blank" href="{{ route('printallupsmasterexpense',['all']) }}"><i style="padding:10px" class="fa fa-print btn btn-primary"></i></a>
                        ...
                        <input type="hidden" name="flag" value="upsMasterExpense">
            ...
            <table id="example" class="display nowrap" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th style="display: none">UPS Master ID</th>
                        <th>
...
<script type="text/javascript">
    ...
    function DatatableInitiate(status = '', expenseType = '') {
        ...
        $('#example').DataTable({
            ...
            "ajax": {
                url: "{{url('ups-master/expense/listbydatatableserverside')}}", // json datasource
            ...
            "createdRow": function(row, data, index) {
                ...
                var url = '<?php echo route("checkupsmasterexpenseoperationfordatatableserverside"); ?>';
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: {
                    ...
                    success: function(data) {
                        ...
                        $('td', row).eq(3).addClass('expandpackage ' + cls);
                        $('td', row).eq(3).attr('style', 'display: block;text-align: center;padding-top: 13px;');
                        $('td', row).eq(3).attr('data-expenseid', expenseId);
                        $('td', row).eq(3).attr('data-rowid', i);
                            ...
                            success: function(data) {
                                if (data.deleted == '0') {
                                    <?php if (checkloggedinuserdata() == 'Cashier') { ?>
                                        var editLink = '<?php echo url("ups-master/expense/viewupsmasterexpenseforcashier"); ?>';
                                        editLink += '/' + expenseId + '/' + upsMasterId;
                                    <?php } else { ?>
                                        if (data.request_by_role == 12 || data.request_by_role == 10) {
                                            var editLink = '<?php echo url("ups-master/expense/editagentupsmasterexpense"); ?>';
                                            editLink += '/' + expenseId + '/flagFromExpenseListing';
                                        } else {
                                            var editLink = '<?php echo url("ups-master/expense/edit"); ?>';
                                            editLink += '/' + expenseId;
                                        }
                                    <?php } ?>
