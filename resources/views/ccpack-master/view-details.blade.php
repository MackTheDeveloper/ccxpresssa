...
<section class="content editupscontainer">
			...
			<div class="" style="float: left;width: 100%;margin-bottom: 10px;">
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="" style="float:left;margin-right: 5px;">
						<a class="btn round orange btn-warning" href="{{route('editccpackmaster',[$model->id])}}">Edit</a>
					</div>
				<?php } ?>
				<div class="" style="float:left;">
					<button class="btn btn-primary" id="upload-file-btn" value="{{url('files/upload',['ccpack-master',$model->id])}}"><i class="fa fa-upload" aria-hidden="true" style="margin-right: 5px"></i>Upload Files</button>
				</div>
			</div>
					...
					<table class="table simpletable display nowrap" id="example3" style="width:100%">
						...
						<tbody>
							...
							foreach ($HouseAWBData as $k => $items) { ?>
								<tr data-editlink="{{ route('viewdetailsccpack',[$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
									<td><?php echo $items->awb_number;  ?></td>
    ...
    float: left">Expenses/Cost</span>
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;    background-color: #3f7b30;border-color: #3f7b30;" class="btn btn-primary" href="{{ route('createccpackmasterexpense',[$id,'flagFromView']) }}">Add Expense</a>
					...
					<tbody>
						<?php if (!empty($dataExpense)) {
							...
							foreach ($dataExpense as $k => $items) {
								$dataClientUsingModuleId = App\Common::getClientDataUsingModuleId('ccpackMaster', $items->ccpack_master_id);
								$items = (object) $items;
								...
								if ($items->request_by_role == 12)
									$edit =  route('editagentccpackmasterexpense', [$items->expense_id, 'flagFromView']);
								else
									$edit =  route('editccpackmasterexpense', [$items->expense_id, 'flagFromView']);
						?>
								<tr data-editlink="{{ $edit }}" id="<?php echo $items->expense_id; ?>" class="edit-row">
									...
									<td><?php $dataCcpack = App\CcpackMaster::getMasterCcpackData($items->ccpack_master_id);
											echo $dataCcpack->file_number;  ?></td>
									<td>{{$items->bl_awb}}</td>
			...
			<div id="div_invoice" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a class="btn btn-primary" style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;background-color: #3f7b30;border-color: #3f7b30;" href="{{ route('createccpackmasterinvoice',[$model->id,'flagFromView']) }}">Add Invoice</a>
					</div>
				<?php } ?>
			</div>
			<div class="detail-container">
				<table id="example" class="simpletable display nowrap" style="width:100%;float: left;">
					...
					<tbody>
						...
						<tr data-editlink="{{ route('editccpackmasterinvoice',[$items->id, 'flagFromView']) }}" id="<?php echo $items->id; ?>" class="edit-row">
							...
							<td>
								<div class='dropdown'>
									...
									$edit =  route('editccpackmasterinvoice', [$items->id, 'flagFromView']);
									?>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class='dropdown'>
									<?php
									$delete =  route('deletefiles', ['ccpack-master', $model->id, serialize($files->file_name)]);
									$download =  route('downloadfiles', ['ccpack-master', $model->id, serialize($files->file_name)]);
									?>
