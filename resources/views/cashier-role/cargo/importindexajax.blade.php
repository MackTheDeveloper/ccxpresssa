        ...
        <tbody>
            ...
            @foreach ($dataCashierCargo as $cargos)
                ...
                <tr data-editlink="{{ route('viewcargodetailforcashier',$cargos->id) }}" id="<?php echo $cargos->id; ?>" class="edit-row">
                    ...
                    <td><?php if($cargos->flag_package_container==1) { $dataPackage = App\CargoPackages::getData($cargos->id); echo 'Weight : '.(!empty($dataPackage->pweight) ? $dataPackage->pweight : '-').' Volume : '.(!empty($dataPackage->pvolume) ? $dataPackage->pvolume : '-').' Pieces : '.(!empty($dataPackage->ppieces) ? $dataPackage->ppieces : '-'); } else {
                        $dataContainer = App\CargoContainers::getData($cargos->id); echo $dataContainer->containerNumbers;
                    } ?></td>
