...
@section('title')
<?php echo $model->id ? 'Update CCPack Master Invoice' : 'Add CCPack Master Invoice'; ?>
@stop

<?php
$permissionCcpackInvoicesCopy = App\User::checkPermission(['copy_ccpack_master_invoices'], '', auth()->user()->id);
$permissionCourierInvoicesDelete = App\User::checkPermission(['delete_ccpack_master_invoices'], '', auth()->user()->id);
...
<section class="content-header">
    <h1><?php echo $model->id ? 'Update CCPack Master Invoice' : 'Add CCPack Master Invoice'; ?></h1>
</section>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('ccpack-master/invoice/update', $model->id);
            else
                $actionUrl = url('ccpack-master/invoice/store');
            ?>
                ...
                <div class="col-md-12" id="container-0" style="margin-left: 25%">
                    <div class="col-md-6">
                        <div class="form-group {{ $errors->has('file_number') ? 'has-error' :'' }}">
                            ...
                            <div class="col-md-12">
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $model->ccpack_master_id; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <input type="hidden" value="<?php echo $model->ccpack_master_id; ?>" id="ccpack_master_id" name="ccpack_master_id">
            ...
            <div class="form-group col-md-12 btm-sub">
                <?php if ($model->id && $permissionCourierInvoicesDelete) { ?>
                    {{-- <a class="btn btn-danger" onclick="return confirm('Are you sure to delete?')" href="{{ route('deleteccpackmasterinvoicefromedit',[$model->id]) }}">Delete</a> --}}
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    $cancleRoute = route('viewdetailsccpackmaster', [$model->ccpack_master_id]);
                } else {
                    $cancleRoute = route('ccpackmasterinvoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("ccpack-master/getccpackmasterinvoiceforinputpicker"); ?>';
        urlztnn2 += '?ccpackMasterId=' + '<?php echo $ccpackMasterId ?>';
        $('#file_number').inputpicker({
        ...
        <?php if (isset($model->ccpack_master_id) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            var urlzt = '<?php echo url("ccpack-master/invoice/getccpackmasterdetailforinvoice"); ?>';
            $.ajax({
                url: urlzt,
                dataType: "json",
                type: 'POST',
                data: {
                    'ccpackMasterId': '<?php echo $model->ccpack_master_id; ?>'
                },
                success: function(data) {
                    console.log(data);
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("ccpackmasterinvoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("ccpack-master/invoice/storeccpackmasterinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
