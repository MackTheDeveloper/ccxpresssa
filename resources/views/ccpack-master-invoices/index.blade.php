...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.customButtonInGridinvoicestatus').click(function() {
            ...
            var ccpackId = $(this).data('ccpackid');
            var thiz = $(this);
            $.ajaxSetup({
                headers: {
    ...
    function DatatableInitiate(fromDate = '', toDate = '') {
        $('#example').DataTable({
            ...
            "scrollX": true,
            "ajax": {
                url: "{{url('ccpack-master/invoice/listbydatatableserverside')}}", // json datasource
                "data": {
                    "fromDate": fromDate,
                    "toDate": toDate
                },
            ...
            "createdRow": function(row, data, index) {
                ...
                var url = '<?php echo url("ccpack-master/invoice/checkoperationfordatatableserverside"); ?>';
                $.ajax({
                    url: url,
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        'invoiceId': invoiceId,
                        'flag': 'getMasterCcpackInvoiceData'
                    },
                    success: function(dataR) {
                        if (dataR.deleted == '0') {
                            var editLink = '<?php echo url("ccpack-master/invoice/viewccpackmasterinvoicedetails"); ?>';
                            editLink += '/' + invoiceId;
