            ...
            @foreach ($invoices as $items)
                <tr data-editlink="{{ route('viewccpackinvoicedetails',[$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
                    ...
                    <td>
                        ...
                        <?php } else { ?>        
                            <li>    
                            <a title="Print Receipt"  target="_blank" href="{{route('printreceiptofinvoicepayment',[$items->id,'invoice','ccpack'])}}">Payment Receipt</i></a>
                            </li>
                        <?php } ?>
                        </ul>
                        </div>
    ...
    $(document).delegate(".to-date-filter", "change", function(){
        $('#loading').show();
         ...
         $.ajax({
                url:urlz,
                type:'POST',
                data:{'fromDate':fromDate,'toDate':toDate,'flag':'ccpackInvoice'},
