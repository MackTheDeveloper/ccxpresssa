...
@section('title')
<?php echo $model->id ? 'Update Aeropost Master Invoice' : 'Add Aeropost Master Invoice'; ?>
@stop

<?php
$permissionAeropostInvoicesCopy = App\User::checkPermission(['copy_aeropost_master_invoices'], '', auth()->user()->id);
$permissionCourierInvoicesDelete = App\User::checkPermission(['delete_aeropost_master_invoices'], '', auth()->user()->id);
$permissionBillingPartyAdd = App\User::checkPermission(['add_billing_party'], '', auth()->user()->id);
if (!empty($model->id)) {
    $countcontainerdetail = App\AeropostInvoiceItemDetails::where('invoice_id', $model->id)->count();
} else {
...
<section class="content-header">
    <h1><?php echo $model->id ? 'Update Aeropost Master Invoice' : 'Add Aeropost Master Invoice'; ?></h1>
</section>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('aeropost-master/invoice/update', $model->id);
            else
                $actionUrl = url('aeropost-master/invoice/store');
            ?>
                ...
                <div class="col-md-12" id="container-0" style="margin-left: 25%">
                    <div class="col-md-6">
                        <div class="form-group {{ $errors->has('file_number') ? 'has-error' :'' }}">
                            ...
                            <div class="col-md-12">
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $model->aeropost_master_id; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <input type="hidden" value="<?php echo $model->aeropost_master_id; ?>" id="aeropost_master_id" name="aeropost_master_id">
            ...
            <div class="form-group col-md-12 btm-sub">
                <?php if ($model->id && $permissionCourierInvoicesDelete) { ?>
                    {{-- <a class="btn btn-danger" onclick="return confirm('Are you sure to delete?')" href="{{ route('deleteaeropostmasterinvoicefromedit',[$model->id]) }}">Delete</a> --}}
                <?php } ?>
                <?php if ($model->id && $permissionAeropostInvoicesCopy) { ?>
                    <a class="btn btn-success" href="{{ route('copyaeropostinvoice',[$model->id]) }}">Copy Invoice</a>
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    $cancleRoute = route('viewdetailsaeropostmaster', [$model->aeropost_master_id]);
                } else {
                    $cancleRoute = route('aeropostmasterinvoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("aeropost-master/getaeropostmasterinvoiceforinputpicker"); ?>';
        urlztnn2 += '?aeropostMasterId=' + '<?php echo $aeropostMasterId ?>';
        $('#file_number').inputpicker({
        ...
        <?php if (isset($model->aeropost_master_id) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            var urlzt = '<?php echo url("aeropost-master/invoice/getaeropostmasterdetailforinvoice"); ?>';
            $.ajax({
                url: urlzt,
                dataType: "json",
                type: 'POST',
                data: {
                    'aeropostMasterId': '<?php echo $model->aeropost_master_id; ?>'
                },
                success: function(data) {
                    console.log(data);
        ...
        $('#file_number').change(function() {
            ...
            if ($(this).val() != '') {
                ...
                $('#aeropost_master_id').val($(this).val());
                var urlzt = '<?php echo url("aeropost-master/invoice/getaeropostmasterdetailforinvoice"); ?>';
                $.ajax({
                    url: urlzt,
                    dataType: "json",
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("aeropostmasterinvoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("aeropost-master/invoice/storeaeropostmasterinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
