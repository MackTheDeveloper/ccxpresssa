...
@section('title')
Cost Items Detail Report - {{$dataCostItem->cost_name}}
@stop
...
<section class="content-header">
    <h1>Cost Items Detail Report - {{$dataCostItem->cost_name}}</h1>
</section>
    ...
    $(document).ready(function() {
        ...
        $('#createInvoiceForm').validate({
            submitHandler: function(form) {
                ...
                var costItemId = '<?php echo $id; ?>';
                if (submitButtonName == 'clsPrint') {
                    var urlztnn = '<?php echo route("printfetchcostitemsdetailreport"); ?>';
                    $.ajax({
                        url: urlztnn,
                        async: false,
                        type: 'POST',
                    ...
                    DatatableInitiate(fromDate, toDate, costItemId);
                }
            },
        });
    })

    function DatatableInitiate(fromDate = '', toDate = '', costItemId = '<?php echo $id; ?>') {
        var i = 1;
        $('#example').DataTable({
            "bDestroy": true,
            "processing": true,
            ...
            "ajax": {
                url: "{{url('reports/listfetchcostitemsdetailreport')}}", // json datasource
