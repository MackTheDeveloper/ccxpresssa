...
<section class="content editupscontainer">
		...
		<div class="box-body">
			...
			<div class="row">
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="col-md-1">
						<a class="btn round orange btn-warning" href="{{route('editupsmaster',[$model->id,$model->ups_operation_type])}}">Edit</a>
					</div>
				<?php } ?>
				<div class="col-md-3">
					...
					<table class="table simpletable display nowrap" id="example3" style="width:100%">
						...
						<tbody>
							...
							foreach ($HouseAWBData as $k => $items) { ?>
								<tr data-editlink="{{ route('viewdetailsups',[$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
    ...
    float: left">Expenses/Cost</span>
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;    background-color: #3f7b30;border-color: #3f7b30;" class="btn btn-primary" href="{{ route('createupsmasterexpense',[$id,'flagFromView']) }}">Add Expense</a>
					...
					<tbody>
						<?php if (!empty($dataExpense)) {
							...
							foreach ($dataExpense as $k => $items) {
								$dataClientUsingModuleId = App\Common::getClientDataUsingModuleId('upsMaster', $items->ups_master_id);
								$items = (object) $items;
								...
								if ($items->request_by_role == 12)
									$edit =  route('editagentupsmasterexpense', [$items->expense_id, 'flagFromView']);
								else
									$edit =  route('editupsmasterexpense', [$items->expense_id, 'flagFromView']);
						?>
								<tr data-editlink="{{ $edit }}" id="<?php echo $items->expense_id; ?>" class="edit-row">
									...
									<td><?php $dataUps = App\UpsMaster::getMasterUpsData($items->ups_master_id);
			...
			<div id="div_invoice" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a class="btn btn-primary" style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;background-color: #3f7b30;border-color: #3f7b30;" href="{{ route('createupsmasterinvoice',[$model->id,'flagFromView']) }}">Add Invoice</a>
					</div>
				<?php } ?>
			</div>
			<div class="detail-container">
				<table id="example" class="simpletable display nowrap" style="width:100%;float: left;">
					...
					<tbody>
						...
						<tr data-editlink="{{ route('editupsmasterinvoice',[$items->id, 'flagFromView']) }}" id="<?php echo $items->id; ?>" class="edit-row">
							...
							<td>
								<div class='dropdown'>
									...
									$edit =  route('editupsmasterinvoice', [$items->id, 'flagFromView']);
									?>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class='dropdown'>
									<?php
									$delete =  route('deletefiles', ['ups-master', $model->id, serialize($files->file_name)]);
									$download =  route('downloadfiles', ['ups-master', $model->id, serialize($files->file_name)]);
									?>
