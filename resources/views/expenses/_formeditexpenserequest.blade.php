...
@section('title')
<?php echo $model->expense_id ? 'Update Cargo File Expense Request' : 'Create File Expense'; ?>
@stop
<?php
...
<section class="content-header">
    <h1><?php echo $model->expense_id ? 'Update Cargo File Expense Request' : 'Add File Expense'; ?></h1>
</section>
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-4">
                        <div class="form-group {{ $errors->has('file_number') ? 'has-error' :'' }}">
                            ...
                            <div class="col-md-12" style="pointer-events: none;opacity: 0.5">
                                <input type="hidden" name="file_number" value="{{$model->cargo_id}}"/>
                                <span class="form-control"><?php echo $model->cargo->file_number; ?></span>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-4">
                        <div class="form-group {{ $errors->has('cargo_id') ? 'has-error' :'' }}">
                            ...
                            <div class="col-md-12" style="pointer-events: none;opacity: 0.5">
                                <input type="hidden" name="cargo_id" value="{{$model->cargo_id}}" />
                                <span class="form-control">
                                    <?php echo $model->cargo->awb_bl_no; ?>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if ($dataCargo->cargo_operation_type != 3) { ?>
                        <div class="col-md-4">
                                ...
                                <tbody>
                                    <?php if (empty($dataExpenseDetails)) { ?>
                                        <tr id="0">
                                            ...
                                            <td><?php echo Form::select('expenseDetails[paid_to][0]', $allUsers, '', ['class' => 'form-control selectpicker fassignee', 'data-live-search' => 'true', 'data-container' => '.expensesubcontainer', 'id' => "paid_to-0"]); ?></td>
    ...
    $(document).ready(function() {
        ...
        $('#createExpenseForm').validate({
            submitHandler: function(form) {
                ...
                var urlz = '<?php echo url("expense/updateagentexpensesbyadmin/$model->expense_id"); ?>';
                $.ajax({
                    url: urlz,
                    type: 'POST',
                    data: formData,
                    success: function(data) {
                        ...
                        <?php if ($flagFromWhere == 'flagFromNotificationPage') { ?>
                            window.location.href = "<?php echo route('expensenotificationoffile', [$model->expense_id, 'cargoExpense']); ?>";
                            <?php } elseif ($flagFromWhere == 'flagFromView') {
                            if ($dataCargo->cargo_operation_type == '3') { ?>
                                window.location.href = '<?php echo route('viewcargolocalfiledetailforcashier', [$model->cargo_id]) ?>';
                            <?php } else { ?>
                                window.location.href = "<?php echo route('viewcargo', [$model->cargo_id, $dataCargo->cargo_operation_type]); ?>";
                            <?php }
                        } else { ?>
