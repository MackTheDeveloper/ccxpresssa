<section class="content" style="font-family: sans-serif;">
    <div class="box box-success" style="width: 100%;margin: 0px auto;">
        <div class="box-body cargo-forms" style="color: #000">
            ...
            <h3 style="background: #ccc;padding:5px;font-weight:normal;">All Expense - <?php echo $dataCargo->file_number; ?></h3>
            <table class="table" id="example1" CELLSPACING="0" CELLPADDING="0" width="100%" class="joureny" style="overflow: visible; text-align:left; font-variant: normal; font-weight: normal;font-size: 14px;background-color: fff;line-height: 20px;font-family: Asap, sans-serif;color: #000;padding: 0;font-style: normal;border:solid 1px hsl(0, 0%, 86%);margin-top:15px; width:900px;border-collapse: collapse">
                <thead>
                    <tr>
                        ...
                        <th width="50px"  style="border-right:1px solid hsl(0, 0%, 86%);border-bottom:1px solid hsl(0, 0%, 86%); border-top: 1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Voucher Number</th>
                        <th width="200px" style="border-right:1px solid hsl(0, 0%, 86%); border-bottom:1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Cost Type</th>
                        ...
                        <th width="100px" style="border-right:1px solid hsl(0, 0%, 86%); border-bottom:1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Billing Party</th>
                        <th width="135px" style="border-right:1px solid hsl(0, 0%, 86%); border-bottom:1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Paid To</th>
                    </tr>
                </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <?php $costData = app('App\Costs')->getCostData($v->expense_type); 
                    ...
                    <td style="border-bottom:1px solid hsl(0, 0%, 86%);border-right:1px solid hsl(0, 0%, 86%); padding:5px;font-family: Asap, sans-serif;font-size:14px; border-collapse: collapse;text-align: center;"><?php $dataClient = app('App\Clients')->getClientData($v->billing_party); echo !empty($dataClient->name) ? $dataClient->name : "-";
                        ?>
                    </td>
            ...
            <?php if(!empty($cargoExpenseDataByVoucher)) { 
                    $kv = 1; 
                    foreach($cargoExpenseDataByVoucher as $ko => $vo)
                        {  $vo = (object) $vo; 
                           $cargoData = App\Cargo::where('id',$vo->cargo_id)->first();
                            $cargoExpenseData = DB::table('expenses')->where('voucher_number',$vo->voucher_number)->where('cargo_id',$vo->cargo_id)->orderBy('expense_id', 'desc')->get();
                            ?>
                                    ...
                                    <div class="mainblk" style="margin-top: 20px; float: none;  width: 842px; padding-bottom: 20px; margin: 0 auto;">
                                        ...
                                        <div class="blockleft" style="margin-bottom: 20px; width: 100%; float: left; padding: 0 20px 0 20px; box-sizing: border-box; border-left: 1px dashed #ccc; border-right: 1px dashed #ccc;">                                            
                                               ...
                                               <div style="float: left;width: 100%; margin: 20px 0 30px 0;">
                                                ...
                                                <div style="float: left;width: 100%; margin-bottom: 10px;">
                                                    ...
                                                    <div style="float: left; width: 68%;"><?php echo !empty($vo->note) ? $vo->note : '-'; ?></div>
                                                </div>  
                                                <div style="float: left;width: 100%; margin-bottom: 10px;">
                                                    <div style="float: left;  width: 25%; font-weight: 600;">Status Note : </div>                            
                                                    <div style="float: left; width: 68%;"><?php echo !empty($vo->expense_request_status_note) ? $vo->expense_request_status_note : '-'; ?></div>
                                                </div> 
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="230px" style="border-right:1px solid hsl(0, 0%, 86%); border-bottom:1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Description</th>
                                                <th width="70px" style="border-right:1px solid hsl(0, 0%, 86%); border-bottom:1px solid hsl(0, 0%, 86%); border-collapse: collapse;padding: 5px;text-align: center;">Amount</th>
                                        ...
                                        <tbody>
                                            <?php $ik = 1; foreach($cargoExpenseData as $k => $v)
                                            { 
                                                $v = (object) $v;
                                                ?>
                                                <tr>
                                            ...
                                            <div style="float: left; width: 100%; margin-top: 70px; bottom: 0; left: 20px;border-bottom: 1px solid #ccc; padding: 15px 0;;">
                                                    <span><?php $dataClient = app('App\Clients')->getClientData($v->billing_party); echo !empty($dataClient->name) ? $dataClient->name : "-";
                                                    ?></span>
                                            </div>
                                        </div>
                                    </div>
                                    <?php if($kv < count($cargoExpenseDataByVoucher)) { ?><pagebreak></pagebreak><?php } ?>
