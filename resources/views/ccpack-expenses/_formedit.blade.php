...
@section('title')
<?php echo $model->expense_id ? 'Update CCpack File Expense Request' : 'Create CCpack File Expense Request'; ?>
@stop
...
<section class="content-header">
    <h1><?php echo $model->expense_id ? 'Update CCpack File Expense Request' : 'Add CCpack File Expense Request'; ?></h1>
</section>
        ...
        <div class="box-body">
            ...
            <div class="expensemaincontainer">
                    ...
                    <div class="col-md-4">
                        <div class="form-group {{ $errors->has('file_number') ? 'has-error' :'' }}">
                            ...
                            <div class="col-md-12" style="pointer-events: none;opacity: 0.5">
                                <?php echo Form::select('file_number', $dataFileNumber, $model->ccpack_id, ['class' => 'form-control selectpicker ffilenumber', 'data-live-search' => 'true', 'placeholder' => 'Select ...']); ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-4">
                        <div class="form-group {{ $errors->has('ccpack_id') ? 'has-error' :'' }}">
                            <div class="col-md-12">
                                <?php echo Form::label('ccpack_id', 'AWB / BL No.', ['class' => 'control-label']); ?>
                            </div>
                            <div class="col-md-12" style="pointer-events: none;opacity: 0.5">
                                <?php echo Form::select('ccpack_id', $dataAwbNos, $model->ccpack_id, ['class' => 'form-control fawb', 'placeholder' => 'Select ...']); ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-12" style="text-align: center;">
                    ...
                    <div class="form-group" style="padding-top: 28px;">
                        ...
                        if ($flagFromWhere == 'flagFromListing')
                            $cancleRoute = route('ccpackexpenses');
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        <?php if (empty($model->billing_party) || $model->expense_id) { ?>
            $('.fbl_awb').val($("#ccpack_id option:selected").html());
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            var urlznn = '<?php echo url("ccpack/getccpackdata"); ?>';
            $.ajax({
                url: urlznn,
                type: 'POST',
                dataType: "json",
        ...
        $('#file_number').change(function() {
            ...
            if ($(this).val() != '') {
                ...
                var ab = $("#ccpack_id option:selected").html();
                $('.fbl_awb').val(ab);
                ...
                var urlzn = '<?php echo url("ccpack/getccpackdata"); ?>';
                $.ajax({
                    url: urlzn,
                    type: 'POST',
                    dataType: "json",
        ...
        $('#createExpenseForm').validate({
            submitHandler: function(form) {
                ...
                var urlz = '<?php echo url("ccpack/expense/update/$model->expense_id"); ?>';
                $.ajax({
                    url: urlz,
                    type: 'POST',
                    data: formData,
                    success: function(data) {
                        ...
                        <?php if ($flagFromWhere == 'flagFromNotificationPage') { ?>
                            window.location.href = "<?php echo route('expensenotificationoffile', [$model->expense_id, 'ccpackExpense']); ?>";
                        <?php } elseif ($flagFromWhere == 'flagFromView') { ?>
                            window.location.href = "<?php echo route('viewdetailsccpack', [$model->ccpack_id]); ?>";
                        <?php } else { ?>
                            window.location.href = "<?php echo route('ccpackexpenses'); ?>";
                        <?php } ?>
