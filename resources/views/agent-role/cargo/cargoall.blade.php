...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        if(DataTableState){
            ...
            // $('#cargoFileType').trigger('change');
            DatatableInitiate(DataTableState.fromDate, DataTableState.toDate, DataTableState.cargoFileType, DataTableState.cargoConsolidateType, DataTableState.fileStatus);
        }else{
            DatatableInitiate();
        }
        ...
        $('#createInvoiceForm').validate({
            submitHandler: function(form) {
                ...
                DatatableInitiate(fromDate, toDate, cargoFileType, cargoConsolidateType, fileStatus);
            },
        });
    })

    function DatatableInitiate(fromDate = '', toDate = '', cargoFileType = '', cargoConsolidateType = '', fileStatus = '') {
        var i = 1;
        var table = $('#example').DataTable({
            ...
            "ajax": {
                url: "{{url('agent/cargo/listbydatatableserverside')}}", // json datasource
                data: function(d) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data.deleted == '0') {
                                    ...
                                    var editLink = '<?php echo url("cargo/viewcargodetailforagent"); ?>';
                                    editLink += '/' + cargoId;
