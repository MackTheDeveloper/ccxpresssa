...
<section class="content-header" style="    display: block;position: relative;top: 0px;">
	<h1 style="font-size: 20px !important;margin-top: 0px;font-weight: 600;">View Detail (<?php echo $model->file_number; ?>)
		...
		<?php } else { ?>
			<div style="float: right;margin-right: 15px;color: green;">File Status : <?php echo isset(Config::get('app.ups_new_scan_status')[!empty($model->aeropost_scan_status) ? $model->aeropost_scan_status : '-']) ? Config::get('app.ups_new_scan_status')[!empty($model->aeropost_scan_status) ? $model->aeropost_scan_status : '-'] : '-';  ?></div>
		<?php } ?>
	</h1>
</section>
	...
	<div class="box box-success">
		<div class="box-body">
			...
			<div class="" style="float: left;width: 100%;margin-bottom: 10px;">
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="" style="float:left;margin-right: 5px;">
						<a class="btn round orange btn-warning" href="{{route('editaeropost',[$model->id])}}">Edit</a>
					</div>
				<?php } ?>
				<div class="" style="float:left;margin-right: 5px;">
					<a style="padding:10px 12px" title="Click here to print" class="btn btn-primary" target="_blank" href="{{route('printaeropostfile', [$model->id])}}"><i class="fa fa-print"></i></a>
				</div>
			...
			<div id="div_expenses" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;    background-color: #3f7b30;border-color: #3f7b30;" class="btn btn-primary" href="{{ route('aeropostexpensecreate',[$id,'flagFromView']) }}">Add Expense</a>
					...
					<tbody>
						<?php if (!empty($dataExpense)) {
							...
							foreach ($dataExpense as $k => $items) {
								$dataClientUsingModuleId = App\Common::getClientDataUsingModuleId('aeropost', $items->aeropost_id);
								$items = (object) $items;
								...
								$edit =  route('aeropostexpenseedit', [$items->expense_id, 'flagFromView']);
						?>
			...
			<div id="div_invoice" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a class="btn btn-primary" style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;background-color: #3f7b30;border-color: #3f7b30;" href="{{ route('createaeropostinvoice',[$model->id,'0','flagFromView']) }}">Add Invoice</a>
					</div>
				<?php } ?>
			</div>
			<div class="detail-container">
				<table id="example" class="simpletable display nowrap" style="width:100%;float: left;">
					...
					<tbody>
						@foreach ($aeropostInvoices as $items)
						<tr data-editlink="{{ route('editaeropostinvoice',[$items->id, 'flagFromView']) }}" id="<?php echo $items->id; ?>" class="edit-row">
							...
							<td>
								<div class='dropdown'>
									...
									$edit =  route('editaeropostinvoice', [$items->id, 'flagFromView']);
									?>
