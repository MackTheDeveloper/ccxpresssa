...
<section class="content-header" style="    display: block;position: relative;top: 0px;">
	<h1 style="font-size: 20px !important;margin-top: 0px;font-weight: 600;">Cash Collection - <?php echo $model->name; ?><a style="line-height:15px;float: right;" class="btn round orange btn-warning" href="{{route('manifestdetailsdeliveryboy',[$model->id])}}">Manifest Details</a><a style="line-height:15px;float: right;margin-right: 10px;" class="btn round orange btn-warning" href="{{route('viewdetailsdeliveryboy',[$model->id])}}">View Details</a></h1>
</section>
<section class="content editupscontainer">
	<div class="box box-success">
		<div class="box-body">
			...
			<div class="db-ups-cash-collection-files">
				<?php echo View::make('delivery-boy.db-ups-cash-collection-files', array('upsFileAssignedToDeliveryBoy' => $upsFileAssignedToDeliveryBoy))->render(); ?>
			</div>
			<div class="db-aeropost-cash-collection-files">
			</div>
			<div class="db-ccpack-cash-collection-files">
			</div>
	...
	$(document).ready(function() {
		...
		$('#form-w0').validate({
			submitHandler: function(form) {
				...
				var urlztnn = '<?php echo url("deliveryboy/filter-files-cash-collection"); ?>';
				$.ajax({
					url: urlztnn,
					async: false,
					type: 'POST',
					...
					success: function(data) {
						if (submitButtonName == 'clsSubmit') {
							if (courierType == 'UPS') {
								$('.db-ups-cash-collection-files').show().html(data);
								$('.db-aeropost-cash-collection-files').hide();
								$('.db-ccpack-cash-collection-files').hide();
							} else if (courierType == 'Aeropost') {
								$('.db-ups-cash-collection-files').hide();
								$('.db-ccpack-cash-collection-files').hide();
								$('.db-aeropost-cash-collection-files').show().html(data);
							} else if (courierType == 'CCPack') {
								$('.db-ups-cash-collection-files').hide();
								$('.db-aeropost-cash-collection-files').hide();
								$('.db-ccpack-cash-collection-files').show().html(data);
							}
						} else {
