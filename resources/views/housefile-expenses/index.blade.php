            ...
            <div class="row" style="margin-bottom:20px">
                ...
                <?php if (checkloggedinuserdata() != 'Agent' && checkloggedinuserdata() != 'Cashier') { ?>
                    <div style="float: right;margin-right: 10px;height: 35px;z-index: 111;top: 18px;">
                        <a style="float:left;margin-right: 10px;" title="Click here to print" target="_blank" href="{{ route('printallhousefileexpense',['all']) }}"><i style="padding:10px" class="fa fa-print btn btn-primary"></i></a>
                        ...
                        <input type="hidden" name="flag" value="housefileExpense">
...
<script type="text/javascript">
    ...
    function DatatableInitiate(status = '', expenseType = '') {
        ...
        $('#example').DataTable({
            ...
            "ajax": {
                url: "{{url('housefileexpense/listbydatatableserverside')}}", // json datasource
            ...
            "createdRow": function(row, data, index) {
                ...
                var url = '<?php echo route("checkcargohousefileexpenseoperationfordatatableserverside"); ?>';
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: {
                            ...
                            success: function(data) {
                                if (data.deleted == '0') {
                                    <?php if (checkloggedinuserdata() == 'Cashier') { ?>
                                        var editLink = '<?php echo url("cashierhousefile/getprintviewsinglehousefileexpensecashier"); ?>';
                                        editLink += '/' + expenseId + '/' + data.house_file_id;
                                    <?php } else if (checkloggedinuserdata() == 'Agent') { ?>
                                        var editLink = '<?php echo url("agenthousefileexpense/edit"); ?>';
                                        editLink += '/' + expenseId;
                                    <?php } else { ?>
                                        if (data.request_by_role == 12 || data.request_by_role == 10) {
                                            var editLink = '<?php echo url("housefileexpense/editrequestedbyagent"); ?>';
                                            editLink += '/' + expenseId;
                                        } else {
                                            var editLink = '<?php echo url("housefileexpense/edit"); ?>';
                                            editLink += '/' + expenseId;
                                        }
                                    <?php } ?>
