                ...
                <tr data-editlink="{{ route('viewupsinvoicedetails',[$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
                    ...
                    <td>
                        ...
                        <?php if($items->payment_status == 'Pending' || $items->payment_status == 'Partial') { ?>
                            ...
                            <?php } else { ?>
                                <li>    
                                        <a title="Print Receipt"  target="_blank" href="{{route('printreceiptofinvoicepayment',[$items->id,'invoice','ups'])}}">Payment Receipt</i></a>
                                        </li>
                                    <?php } ?>
                        </ul>
                        </div>
    ...
    $(document).delegate(".to-date-filter", "change", function(){
        $('#loading').show();
         ...
         $.ajax({
                url:urlz,
                type:'POST',
                data:{'fromDate':fromDate,'toDate':toDate,'flag':'upsInvoice'},
