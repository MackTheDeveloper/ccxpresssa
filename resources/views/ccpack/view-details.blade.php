...
<section class="content-header" style="    display: block;position: relative;top: 0px;">
	<h1 style="font-size: 20px !important;margin-top: 0px;font-weight: 600;">View Detail (<?php echo $model->file_number; ?>)
		...
		<?php } else { ?>
			<div style="float: right;margin-right: 15px;color: green;">File Status : <?php echo isset(Config::get('app.ups_new_scan_status')[!empty($model->ccpack_scan_status) ? $model->ccpack_scan_status : '-']) ? Config::get('app.ups_new_scan_status')[!empty($model->ccpack_scan_status) ? $model->ccpack_scan_status : '-'] : '-';  ?></div>
		<?php } ?>
	</h1>
</section>
		...
		<div class="box-body">
			...
			<div class="" style="float: left;width: 100%;margin-bottom: 10px;">
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="" style="float:left;margin-right: 5px;">
						<a class="btn round orange btn-warning" href="{{route('editccpack',[$model->id])}}">Edit</a>
					</div>
				<?php } ?>
				<div class="" style="float:left;margin-right: 5px;">
					<a style="padding:10px 12px" title="Click here to print" class="btn btn-primary" target="_blank" href="{{route('printccpackfile', [$model->id])}}"><i class="fa fa-print"></i></a>
				</div>
				<div class="" style="float:left;">
					<button class="btn btn-primary" id="upload-file-btn" value="{{url('files/upload',['ccpack',$model->id])}}"><i class="fa fa-upload" aria-hidden="true" style="margin-right: 5px"></i>Upload Files</button>
				</div>
			</div>
			...
			<div class="detail-container basicDetaiCls">
				...
				<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
					...
					<span class="viewblk2"><?php echo $model->shipper_telephone; ?></span>
				</div>
				<div style="float: left;width: 50%; margin-bottom: 10px;">
					<span class="viewblk1">Consignee Phone No : </span>
					<span class="viewblk2"><?php echo $model->consignee_telephone; ?></span>
				</div>
				<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
				...
				<div style="float: left;width: 50%; margin-bottom: 10px;">
					...
					<span class="viewblk2"><?php echo !empty($model->no_of_pcs) ? (int) $model->no_of_pcs : '-' ?></span>
				</div>
				<div class="newline" style="float: left;width: 50%; margin-bottom: 10px;">
			...
			<div id="div_expenses" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;    background-color: #3f7b30;border-color: #3f7b30;" class="btn btn-primary" href="{{ route('ccpackexpensecreate',[$id,'flagFromView']) }}">Add Expense</a>
						<div style="float: left">
							<a title="Click here to print" target="_blank" href="{{ route('printallccpackexpense',[$model->id]) }}"><i style="background-color: #3f7b30;border-color: #3f7b30;" class="fa fa-print btn btn-primary"></i></a>
						</div>
					</div>
				<?php } ?>
			</div>
					...
					<tbody>
						<?php if (!empty($dataExpense)) {
							...
							foreach ($dataExpense as $k => $items) {
								$dataClientUsingModuleId = App\Common::getClientDataUsingModuleId('ccpack', $items->ccpack_id);
								$items = (object) $items;
								...
								$edit =  route('ccpackexpenseedit', [$items->expense_id, 'flagFromView']);
						?>
			...
			<div id="div_invoice" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a class="btn btn-primary" style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;background-color: #3f7b30;border-color: #3f7b30;" href="{{ route('createccpackinvoices',[$model->id,'0','flagFromView']) }}">Add Invoice</a>
					</div>
				<?php } ?>
			</div>
			<div class="detail-container">
				<table id="example" class="simpletable display nowrap" style="width:100%;float: left;">
					...
					<tbody>
						@foreach ($ccpackInvoices as $items)
						<tr data-editlink="{{ route('editccpackinvoice',[$items->id, 'flagFromView']) }}" id="<?php echo $items->id; ?>" class="edit-row">
							...
							<td>
								<div class='dropdown'>
									...
									$edit =  route('editccpackinvoice', [$items->id, 'flagFromView']);
									?>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class='dropdown'>
									<?php
									$delete =  route('deletefiles', ['ccpack', $model->id, serialize($files->file_name)]);
									$download =  route('downloadfiles', ['ccpack', $model->id, serialize($files->file_name)]);
									?>
