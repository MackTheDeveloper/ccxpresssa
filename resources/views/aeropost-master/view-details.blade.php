...
<section class="content editupscontainer">
			...
			<div class="" style="float: left;width: 100%;margin-bottom: 10px;">
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="" style="float:left;margin-right: 5px;">
						<a class="btn round orange btn-warning" href="{{route('editaeropostmaster',[$model->id])}}">Edit</a>
					</div>
				<?php } ?>
				<div class="" style="float:left;">
					<button class="btn btn-primary" id="upload-file-btn" value="{{url('files/upload',['aeropost-master',$model->id])}}"><i class="fa fa-upload" aria-hidden="true" style="margin-right: 5px"></i>Upload Files</button>
				</div>
			</div>
						...
						<tbody>
							...
							foreach ($HouseAWBData as $k => $items) { ?>
								<tr data-editlink="{{ route('viewdetailsaeropost',[$items->id]) }}" id="<?php echo $items->id; ?>" class="edit-row">
									<td><?php echo $items->tracking_no;  ?></td>
									<td><?php echo $items->file_number; ?></td>
									<td><?php echo !empty($items->consignee) ? App\Ups::getConsigneeName($items->consignee) : '-' ?></td>
									<td><?php echo !empty($items->from_location) ? $items->from_location : '-' ?></td>
								</tr>
							<?php $i++;
							} ?>
						</tbody>
    ...
    float: left">Expenses/Cost</span>
				&nbsp;&nbsp;&nbsp;&nbsp;<span style="display:none;margin-top: 3px;float: left;margin-left: 15px;"><b>Total:</b></span><span style="display:none;margin-top: 3px;float: left">&nbsp;
					<?php $totalExpense = App\Expense::getTotalExpenseOfUps($id);
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;    background-color: #3f7b30;border-color: #3f7b30;" class="btn btn-primary" href="{{ route('createaeropostmasterexpense',[$id,'flagFromView']) }}">Add Expense</a>
						<div style="float: left">
							<a title="Click here to print" target="_blank" href="{{ route('printallaeropostexpense',[$model->id]) }}"><i style="background-color: #3f7b30;border-color: #3f7b30;" class="fa fa-print btn btn-primary"></i></a>
						</div>
					</div>
				<?php } ?>
			</div>
					...
					<tbody>
						<?php if (!empty($dataExpense)) {
							...
							foreach ($dataExpense as $k => $items) {
								$dataClientUsingModuleId = App\Common::getClientDataUsingModuleId('aeropostMaster', $items->aeropost_master_id);
								$items = (object) $items;
								...
								if ($items->request_by_role == 12)
									$edit =  route('editagentaeropostmasterexpense', [$items->expense_id, 'flagFromView']);
								else
									$edit =  route('editaeropostmasterexpense', [$items->expense_id, 'flagFromView']);
						?>
								<tr data-editlink="{{ $edit }}" id="<?php echo $items->expense_id; ?>" class="edit-row">
									...
									<td><?php $dataAeropost = App\AeropostMaster::getMasterAeropostData($items->aeropost_master_id);
											echo $dataAeropost->file_number;  ?></td>
									<td>{{$items->bl_awb}}</td>
			...
			<div id="div_invoice" class="notes box-s" style="margin-top:12px;margin-bottom:-15px;"><span style="margin-top: 3px;
				...
				<?php if ($model->file_close != 1 && $model->deleted != 1) { ?>
					<div class="box-span">
						<a class="btn btn-primary" style="width: auto;float: left;color: #fff;margin-right: 10px;padding-top: 3px;height: 30px;background-color: #3f7b30;border-color: #3f7b30;" href="{{ route('createaeropostmasterinvoice',[$model->id,'flagFromView']) }}">Add Invoice</a>
					</div>
				<?php } ?>
			</div>
			<div class="detail-container">
				<table id="example" class="simpletable display nowrap" style="width:100%;float: left;">
					...
					<tbody>
						...
						<tr data-editlink="{{ route('editaeropostmasterinvoice',[$items->id, 'flagFromView']) }}" id="<?php echo $items->id; ?>" class="edit-row">
							...
							<td>
								<div class='dropdown'>
									...
									$edit =  route('editaeropostmasterinvoice', [$items->id, 'flagFromView']);
									?>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class='dropdown'>
									<?php
									$delete =  route('deletefiles', ['aeropost-master', $model->id, serialize($files->file_name)]);
									$download =  route('downloadfiles', ['aeropost-master', $model->id, serialize($files->file_name)]);
									?>
