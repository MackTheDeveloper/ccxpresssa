...
<?php echo $model->id ? 'Update UPS Master Invoice' : 'Add UPS Master Invoice'; ?>
<?php $__env->stopSection(); ?>

<?php
$permissionUpsInvoicesCopy = App\User::checkPermission(['copy_ups_master_invoices'], '', auth()->user()->id);
$permissionCourierInvoicesDelete = App\User::checkPermission(['delete_ups_master_invoices'], '', auth()->user()->id);
$permissionBillingPartyAdd = App\User::checkPermission(['add_billing_party'], '', auth()->user()->id);
if (!empty($model->id)) {
    $countcontainerdetail = App\UpsInvoiceItemDetails::where('invoice_id', $model->id)->count();
} else {
...
<?php echo $__env->make('menus.ups-invoice', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<?php $__env->stopSection(); ?>
...
<section class="content-header">
    <h1><?php echo $model->id ? 'Update Ups Master Invoice' : 'Add Ups Master Invoice'; ?></h1>
</section>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('ups-master/invoice/update', $model->id);
            else
                $actionUrl = url('ups-master/invoice/store');
            ?>
            ...
            <div class="invoice_cotainer">
                ...
                <div class="col-md-12" id="container-0" style="margin-left: 25%">
                    <div class="col-md-6">
                        <div class="form-group <?php echo e($errors->has('file_number') ? 'has-error' :''); ?>">
                            ...
                            <div class="col-md-12">
                                ...
                                ?>
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $model->ups_master_id; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <input type="hidden" value="<?php echo $model->ups_master_id; ?>" id="ups_master_id" name="ups_master_id">
            ...
            <div class="form-group col-md-12 btm-sub">
                <?php if ($model->id && $permissionCourierInvoicesDelete) { ?>
                    
                <?php } ?>
                <?php if ($model->id && $permissionUpsInvoicesCopy) { ?>
                    <a class="btn btn-success" href="<?php echo e(route('copyupsinvoice',[$model->id])); ?>">Copy Invoice</a>
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    $cancleRoute = route('viewdetailsupsmaster', [$model->ups_master_id]);
                } else {
                    $cancleRoute = route('upsmasterinvoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("ups-master/getupsmasterinvoiceforinputpicker"); ?>';
        urlztnn2 += '?upsMasterId=' + '<?php echo $upsMasterId ?>';
        $('#file_number').inputpicker({
        ...
        <?php if (isset($model->ups_master_id) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            var urlzt = '<?php echo url("ups-master/invoice/getupsmasterdetailforinvoice"); ?>';
            $.ajax({
                url: urlzt,
                dataType: "json",
                type: 'POST',
                data: {
                    'upsMasterId': '<?php echo $model->ups_master_id; ?>'
                },
                success: function(data) {
                    console.log(data);
                    /* if (data.fc == '1')
                    ...
                    $('.hawb_no').val(data.tracking_number);
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("upsmasterinvoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("ups-master/invoice/storeupsmasterinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/ups-master-invoices/form.blade.php ENDPATH**/ ?>