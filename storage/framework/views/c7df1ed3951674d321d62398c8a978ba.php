...
<?php if (empty($model->flag_invoice)) {
    echo $model->id ? 'Update House File Invoice' : 'Add House File Invoice';
} else {
    echo $model->id ? 'Update House File Old Invoice' : 'Add House File Old Invoice';
} ?>
<?php $__env->stopSection(); ?>

<?php
//$flagModule = empty($model->id) ? $flagModule : $model->housefile_module;
if (empty($flagModule))
    $flagModule = 'cargo';
...
if (!empty($model->id)) {
    $countcontainerdetail = App\HouseFileInvoiceItemDetails::where('invoice_id', $model->id)->count();
} else {
...
<?php if (auth()->user()->department == 14) { ?>
    <?php if (empty($model->flag_invoice)) { ?>
        ...
        <?php echo $__env->make('menus.warehouse-cargo-invoice', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        <?php $__env->stopSection(); ?>
    <?php } else { ?>
        <?php $__env->startSection('breadcrumbs'); ?>
...
<?php } else if (auth()->user()->department == 11) { ?>
    <?php if (empty($model->flag_invoice)) { ?>
        <?php $__env->startSection('breadcrumbs'); ?>
        <?php echo $__env->make('menus.cashier-warehouse-cargo-invoice', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        <?php $__env->stopSection(); ?>
    <?php } else { ?>
        <?php $__env->startSection('breadcrumbs'); ?>
        <?php echo $__env->make('menus.old-invoices', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('houseinvoices/update', $model->id);
            else
                $actionUrl = url('houseinvoices/store');
            ?>
            <?php echo e(Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form','id'=>'createInvoiceForm','autocomplete'=>'off'))); ?>
            ...
            <input type="hidden" class="housefile_module" name="housefile_module" value="<?php echo $flagModule; ?>">

            <div class="invoice_cotainer">

                <div class="col-md-12" id="container-0" style="">
                    <div class="col-md-5">
                        <div class="form-group <?php echo e($errors->has('file_number') ? 'has-error' :''); ?>">
                            ...
                            <div class="col-md-12">
                                <?php
                                if ($flagModule == 'cargo')
                                    $fileNumberId =  $model->cargo_id;
                                else if ($flagModule == 'ups')
                                    $fileNumberId =  $model->ups_id;
                                else if ($flagModule == 'aeropost')
                                    $fileNumberId =  $model->aeropost_id;
                                else if ($flagModule == 'ccpack')
                                    $fileNumberId =  $model->ccpack_id;
                                ?>
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $fileNumberId; ?>">
                            </div>
                        </div>
                    </div>
                    <?php if ($flagModule == 'cargo') { ?>
                        <div class="col-md-5">
                            <div class="form-group <?php echo e($errors->has('hawb_hbl_no') ? 'has-error' :''); ?>">
                                <?php echo Form::label('hawb_hbl_no', 'House File No.', ['class' => 'col-md-12  control-label']); ?>
                                <div class="col-md-12">
                                    <input class="form-control ffilenumber fhawbfilenumber" name="hawb_hbl_no" id="hawb_hbl_no" value="<?php echo $model->hawb_hbl_no; ?>">
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    ...
                    <input type="hidden" value="<?php echo $model->cargo_id; ?>" id="cargo_id" name="cargo_id">
                    <input type="hidden" value="<?php echo $model->ups_id; ?>" id="ups_id" name="ups_id">
                    <input type="hidden" value="<?php echo $model->aeropost_id; ?>" id="aeropost_id" name="aeropost_id">
                ...
                <div class="col-md-12" style="margin-top: 20px">
                    <div class="sec1" style="float: left;width: 100%">
                        <div class="d1 col-md-2">
                            <?php echo e(Form::image('images/invoice_logo.png', 'alt text', array('class' => 'css-class'))); ?>
            ...
            <div class="form-group col-md-12 btm-sub">
                <?php if ($model->id && $permissionCargoInvoicesDelete) { ?>
                    
                <?php } ?>
                <?php if ($model->id && $permissionCargoInvoicesCopy) { ?>
                    <a class="btn btn-success" href="<?php echo e(route('copyhouseinvoice',[$model->id])); ?>">Copy Invoice</a>
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    $cancleRoute = route('viewhawbfile', [$model->hawb_hbl_no]);
                } else if ($model->flag_invoice == 'old') {
                    $cancleRoute = route('oldinvoices');
                } else {
                    $cancleRoute = route('housefileinvoices', [$flagModule]);
                }
                ?>

                <a class="btn btn-danger" href="<?php echo e($cancleRoute); ?>" title="">Cancel</a>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title modal-title-block text-center primecolor">Add Client</h3>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("houseinvoices/getcargoinhousefileinvoiceforinputpicker"); ?>';
        urlztnn2 += '?flagModule=' + '<?php echo $flagModule ?>' + '&moduleId=' + '<?php echo $moduleId ?>';
        $('#file_number').inputpicker({
        ...
        var urlztnn2 = '<?php echo url("houseinvoices/getcargohousefileinvoiceforinputpicker"); ?>';
        urlztnn2 += '?houseId=' + '<?php echo $model->hawb_hbl_no ?>';
        $('#hawb_hbl_no').inputpicker({
        ...
        <?php if (isset($model->hawb_hbl_no) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });
            var urlzt = '<?php echo url("houseinvoices/gethousedetailforinvoice"); ?>';
            $.ajax({
                url: urlzt,
                dataType: "json",
                type: 'POST',
                data: {
                    'id': '<?php echo $model->hawb_hbl_no; ?>'
                },
                success: function(data) {
                    $('.hbill_to').val(data.billing_party);
                    //$('.hbill_to').val(data.consignee_name);
                    $('#bill_to').trigger('change');
                    $('.hshipper').val(data.shipper_full_name + (data.shipper_address == null ? '' : ', ' + data.shipper_address));
                    $('.hconsignee_address').val(data.consignee_full_name + (data.consignee_address == null ? '' : ', ' + data.consignee_address));
                    $('.hfile_no').val(data.file_number);
                    if (data.cargo_operation_type == '1')
                        $('.hawb_no').val(data.hawb_hbl_no);
                    else
                        $('.hawb_no').val(data.export_hawb_hbl_no);
                    if (data.cargo_operation_type == 1)
                        $('.htype_flag').val('IMPORT');
                    else
                        $('.htype_flag').val('EXPORT');
                    $('.hweight').val(data.pweight);
                    $('#cargo_id').val(data.MasterCargoFile);
                    $('#loading').hide();
                }
            });
        <?php } ?>
        ...
        $('#file_number').change(function() {
            ...
            <?php if ($flagModule == 'ups') { ?>
                if ($(this).val() != '') {
                    $('#ups_id').val($(this).val());
                    var urlzt = '<?php echo url("upsinvoice/getupsdetailforinvoice"); ?>';
                    $.ajax({
                        url: urlzt,
                        dataType: "json",
                        type: 'POST',
                        ...
                        success: function(data) {
                            ...
                            if (data.fd == '1')
                                $('.hbill_to').val('UPS');
                            if (data.pp == '1')
                                $('.hbill_to').val(data.shipper_name);

                            $('#bill_to').trigger('change');
                            $('.hshipper').val(data.shipper_full_name);
                            $('.hconsignee_address').val(data.consignee_full_name);
                            $('.hfile_no').val(data.file_number);
            ...
            <?php if ($flagModule == 'aeropost') { ?>
                if ($(this).val() != '') {
                    ...
                    var urlzt = '<?php echo url("aeropostinvoices/getaeropostdetailforinvoice"); ?>';
                    $.ajax({
                        url: urlzt,
                        dataType: "json",
                        type: 'POST',
                        ...
                        success: function(data) {
                            //$('.hbill_to').val('203');
                            $(".hbill_to option:contains('Aeropost')").attr('selected', 'selected');
                            $('#bill_to').trigger('change');
                            $('.hshipper').val(data.from_location);
                            var urlzt = '<?php echo url("clients/getclientdata"); ?>';
                            $.ajax({
                                url: urlzt,
                                dataType: "json",
                                type: 'POST',
                                data: {
                                    'clientId': data.consignee
                                },
                                success: function(dataN) {
                                    $('.hconsignee_address').val(dataN.company_name);
                                }
                            })
                            $('.hfile_no').val(data.file_number);
                            $('.hawb_no').val(data.tracking_no);
                            $('.htype_flag').val('IMPORT');
                            $('.hweight').val(data.real_weight);
                            $('.selectpicker').selectpicker('refresh');
                            $('#loading').hide();
                        }
                    });
        ...
        $('#createInvoiceForm').validate({
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("housefileinvoices", $flagModule) ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("houseinvoices/storehouseinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
            ...
            errorPlacement: function(error, element) {
                if (element.attr("name") == "bill_to") {
                    ...
                    error.insertAfter(pos);
                } else if (element.attr("name") == "hawb_hbl_no") {
                    var pos = $('.inputpicker-div input.fhawbfilenumber');
                    error.insertAfter(pos);
                    $('label#hawb_hbl_no-error').remove();
                } else {
        ...
        $(document).on("change", "select.feesnamefld", function(e) {
            ...
            if (billingId != '') {
                ...
                <?php if ($flagModule == 'cargo') { ?>
                    if ($('#cargo_id').val() != '')
                        var moduleId = $('#cargo_id').val();
                    else
                        var moduleId = '';
                    var hawbId = $('#hawb_hbl_no').val();
                <?php } else if ($flagModule == 'ups') { ?>
                    if ($('#ups_id').val() != '')
                        var moduleId = $('#ups_id').val();
                    else
                        var moduleId = '';
                <?php } else if ($flagModule == 'aeropost') { ?>
                    if ($('#aeropost_id').val() != '')
                        var moduleId = $('#aeropost_id').val();
                    else
                        var moduleId = '';
                <?php } else if ($flagModule == 'ccpack') { ?>
                    if ($('#ccpack_id').val() != '')
                        var moduleId = $('#ccpack_id').val();
                    else
                        var moduleId = '';
                <?php } ?>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.billingCode == 'SCD') {
                            $('#quantity-' + id).val(data.billingDays);
                        }
                        if (data.billingCode == 'SCW') {
                            $('#quantity-' + id).val(data.billingWeeks);
                        }
                        if (data.billingCode == 'SCM') {
                            $('#quantity-' + id).val(data.billingMonths);
                        }
                        if (data.billingCode == 'SCC') {
                            $('#quantity-' + id).val(data.billingCustomDays);
                            $('#unit_price-' + id).val(data.billingCustomStorageCharage);
                            $('.unitpricefld').trigger('focusout');
                        }
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/housefile-invoices/form.blade.php ENDPATH**/ ?>