...
<?php if (empty($model->flag_invoice)) {
    echo $model->id ? 'Update UPS Invoice' : 'Add UPS Invoice';
} else {
    echo $model->id ? 'Update UPS Old Invoice' : 'Add UPS Old Invoice';
} ?>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('upsinvoice/update', $model->id);
            else
                $actionUrl = url('upsinvoice/store');
            ?>
                    ...
                    <div class="col-md-6">
                        <div class="form-group <?php echo e($errors->has('file_number') ? 'has-error' :''); ?>">
                            ...
                            <div class="col-md-12">
                                ...
                                ?>
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $model->ups_id; ?>">
                            </div>
                        </div>
                    </div>
            ...
            <div class="form-group col-md-12 btm-sub">
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    $cancleRoute = route('viewdetailsups', [$model->ups_id]);
                } else if ($model->flag_invoice == 'old') {
                    $cancleRoute = route('oldinvoices');
                } else {
                    $cancleRoute = route('upsinvoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        /* $(document).delegate("#file_number", "keyup", function(){
            var valueOfText = $(this).val();
            if (valueOfText.length >= 3) {
                var urlztnn2 = '<?php //echo url("upsinvoice/getupsinvoiceforinputpicker"); 
                                ?>';
                $.ajax({
                    url: urlztnn2,
                    type: 'POST',
                    data: {
                        'upsId': '<?php //echo $upsId; 
                                    ?>',
                        'valueOfText' : valueOfText
                    },
                    success: function(response) {
                        var allInvoices = JSON.parse(response.NdataFileNumber);
                        $('#file_number').inputpicker({
        ...
        <?php if (isset($model->ups_id) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("upsinvoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("upsinvoice/storeupsinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/upsinvoices/form.blade.php ENDPATH**/ ?>