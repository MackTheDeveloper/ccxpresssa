...
<?php if (checkloggedinuserdata() == 'Agent') { ?>
    ...
    <?php echo $__env->make('menus.agent-cargo-expense', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php $__env->stopSection(); ?>
<?php } else if (checkloggedinuserdata() == 'Cashier') { ?>
    <?php $__env->startSection('breadcrumbs'); ?>
    <?php echo $__env->make('menus.cashier-cargo-expense', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php $__env->stopSection(); ?>
<?php } else { ?>
    <?php $__env->startSection('breadcrumbs'); ?>
    <?php echo $__env->make('menus.cargo-expense', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php $__env->stopSection(); ?>
<?php } ?>
    ...
    <div class="box box-success cargocontainer">
        <div class="box-body">
            <div class="row" style="margin-bottom:20px">
                <?php echo e(Form::open(array('url' => '','class'=>'form-horizontal create-form','id'=>'filterExpenses','autocomplete'=>'off'))); ?>
                ...
                <?php if (checkloggedinuserdata() != 'Agent' && checkloggedinuserdata() != 'Cashier') { ?>
                    <div style="float: right;margin-right: 10px;height: 35px;z-index: 111;top: 18px;">
                        <a style="float:left;margin-right: 10px;" title="Click here to print" target="_blank" href="<?php echo e(route('getprintallexpense',['all'])); ?>"><i style="padding:10px" class="fa fa-print btn btn-primary"></i></a>
                        <?php $actionUrl = route('approveallselectedexpense'); ?>
                        <?php echo e(Form::open(array('url' => $actionUrl,'class'=>'form-horizontal create-form create-form-expenenseinbasiccargo','id'=>'createExpenseForm','autocomplete'=>'off','style'=>'float:left;width:auto'))); ?>

                        <?php echo e(csrf_field()); ?>

                        <input type="hidden" name="ids" class="ids" value="">
                        <input type="hidden" name="flag" value="cargoExpense">
...
<script type="text/javascript">
    ...
    function DatatableInitiate(status = '', expenseType = '') {
        ...
        $('#example').DataTable({
            ...
            "ajax": {
                url: "<?php echo e(url('expense/listbydatatableserverside')); ?>", // json datasource
                data: function(d) {
                    d.status = status;
                            ...
                            success: function(data) {
                                if (data.deleted == '0') {
                                    <?php if (checkloggedinuserdata() == 'Cashier') { ?>
                                        ...
                                        editLink += '/' + expenseId + '/' + data.cargo_id;
                                    <?php } else if (checkloggedinuserdata() == 'Agent') { ?>
                                        var editLink = '<?php echo url("cargo/editeagentexpenses"); ?>';
                                        editLink += '/' + expenseId + '/flagFromExpenseListing';
                                    <?php } else { ?>
                                        if (data.request_by_role == 12 || data.request_by_role == 10) {
                                            var editLink = '<?php echo url("expense/editagentexpensesbyadmin"); ?>';
                                            editLink += '/' + expenseId + '/flagFromExpenseListing';
                                        } else {
                                            var editLink = '<?php echo url("expense/editexpensevoucher"); ?>';
                                            editLink += '/' + expenseId + '/flagFromExpenseListing';
                                        }
                                    <?php } ?>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/expenses/index.blade.php ENDPATH**/ ?>