...
<?php if (empty($model->flag_invoice)) {
    echo $model->id ? 'Update Cargo Invoice' : 'Add Cargo Invoice';
} else {
    echo $model->id ? 'Update Cargo Old Invoice' : 'Add Cargo Old Invoice';
} ?>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('invoices/update', $model->id);
            else
                $actionUrl = url('invoices/store');
            ?>
            ...
            <div class="invoice_cotainer">
                ...
                <div class="col-md-12" id="container-0" style="margin-left: 0%">
                    <?php if (isset($dataCargoSingle) && $dataCargoSingle->consolidate_flag == 1) { ?>
                        <div class="col-md-5">
                        ...
                        <div class="col-md-5">
                            <div class="form-group <?php echo e($errors->has('hawb_hbl_no') ? 'has-error' :''); ?>">
                                <?php echo Form::label('hawb_hbl_no', 'HAWB', ['class' => 'col-md-12  control-label']); ?>
                                <div class="col-md-12">
            ...
            <div class="form-group col-md-12 btm-sub">
                ...
                <?php if ($model->id && $permissionCargoInvoicesCopy) { ?>
                    <a class="btn btn-success" href="<?php echo e(route('copyinvoice',[$model->id])); ?>">Copy Invoice</a>
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    if ($dataCargoSingle->cargo_operation_type == 3)
                        $cancleRoute = route('viewcargolocalfiledetailforcashier', [$model->cargo_id]);
                    else
                        $cancleRoute = route('viewcargo', [$model->cargo_id, $dataCargoSingle->cargo_operation_type]);
                } else if ($model->flag_invoice == 'old') {
                    $cancleRoute = route('oldinvoices');
                } else {
                    $cancleRoute = route('invoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("invoice/getcargoinvoiceforinputpicker"); ?>';
        urlztnn2 += '?cargoId=' + '<?php echo $cargoId ?>';
        $('#file_number').inputpicker({
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("invoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("invoices/storecargoinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/invoices/form.blade.php ENDPATH**/ ?>