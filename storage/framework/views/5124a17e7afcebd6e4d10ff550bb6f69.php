...
<?php if (empty($model->flag_invoice)) {
    echo $model->id ? 'Update Aeropost Invoice' : 'Add Aeropost Invoice';
} else {
    echo $model->id ? 'Update Aeropost Old Invoice' : 'Add Aeropost Old Invoice';
} ?>
...
<?php if (empty($model->flag_invoice)) { ?>
    ...
    <?php echo $__env->make('menus.aeropost-invoice', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php $__env->stopSection(); ?>
<?php } else { ?>
    <?php $__env->startSection('breadcrumbs'); ?>
...
<section class="content">
    <div class="box box-success" style="float: left;">
        <div class="box-body">
            <?php
            if ($model->id)
                $actionUrl = url('aeropostinvoices/update', $model->id);
            else
                $actionUrl = url('aeropostinvoices/store');
            ?>
                    ...
                    <div class="col-md-6">
                        <div class="form-group <?php echo e($errors->has('file_number') ? 'has-error' :''); ?>">
                            ...
                            <div class="col-md-12">
                                ...
                                ?>
                                <input class="form-control ffilenumber" name="file_number" id="file_number" value="<?php echo $model->aeropost_id; ?>">
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-12" style="margin-top: 20px">
                    ...
                    <div class="sec6" style="float: left;width: 100%;margin-top: 10px">
                        <div class="col-md-4">
                            ...
                            <?php echo Form::text('memo', $model->memo, ['class' => 'form-control hmemo']); ?></div>
            ...
            <div class="form-group col-md-12 btm-sub">
                <?php if ($model->id && $permissionAeropostInvoicesDelete) { ?>
                    
                <?php } ?>
                <?php if ($model->id && $permissionAeropostInvoicesCopy) { ?>
                    <a class="btn btn-success" href="<?php echo e(route('copyaeropostinvoice',[$model->id])); ?>">Copy Invoice</a>
                <?php } ?>
                ...
                <?php if ($flagFromWhere == 'flagFromView') {
                    ...
                    $cancleRoute = route('aeropostinvoices');
                }
                ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var urlztnn2 = '<?php echo url("aeropostinvoices/getaeropostinvoiceforinputpicker"); ?>';
        urlztnn2 += '?aeropostId=' + '<?php echo $aeropostId ?>';
        $('#file_number').inputpicker({
        ...
        <?php if (isset($model->aeropost_id) && !isset($id)) { ?>
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.hweight').val(data.shipment_real_weight);
                            ...
                            success: function(data) {
                                if (data.exist == 1) {
                                    if (confirm("Invoice with Bill No " + bNo + " already created. Do you want to continue with incremented number ?")) {
                                        ...
                                        goAhead = 0;
                                        window.location.href = '<?php echo route("aeropostinvoices") ?>';
                                        event.preventDefault();
                                        return false;
                                    }
                                }
                        ...
                        success: function(data) {
                            ...
                            if (data.cash_credit == 'Credit' && parseFloat(finalTotal) > parseFloat(clientLimit)) {
                                ...
                                if (confirm("Total amount is exceeding client limit, Do you want to proceed?")) {
                                    //form.submit();
                                    var urlznt = '<?php echo url("aeropostinvoices/storeaeropostinvoiceandprint"); ?>';
                                    $.ajax({
                                        url: urlznt,
                                        type: 'POST',
                                        data: formData,
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.custom', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/php/cargo/resources/views/aeropost-invoices/form.blade.php ENDPATH**/ ?>